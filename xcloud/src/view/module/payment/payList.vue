    ...
    <Card v-show="listModel">
      ...
      <Table border :columns="columns" :data="data" :loading="loadTable">
        ...
        <template slot="action" slot-scope="{ row }">
          ...
          <Button type="info" ghost @click="reissueNotice(row.payOrderId)" v-show="row.notifyStatus===1">补发通知</Button>
        </template>
      </Table>
      <Page :total="pageInfo.total" show-elevator show-sizer transfer :current="pageInfo.pageIndex" :page-size="pageInfo.pageSize" show-total @on-change="handlePage" @on-page-size-change="handlePageSize" />
    </Card>
    <Card v-show="!listModel">
      <div>
        <Row>
        ...
        <Row>
          <Col span="8">订单状态：{{listInfo.status===0?'订单生成':listInfo.status===1?'支付中':listInfo.status===2?'支付成功':listInfo.status===3?'业务处理完成':'未知状态'}}</Col>
        </Row>
        <!-- <Row>
...
<script>
import { getPayList, getPayInfo, getAllpayWay, reissueNotice } from '@/api/payment'
...
export default {
  ...
  methods: {
    ...
    getList () {
      ...
      if (obj.status === undefined) self.pageInfo.status = ''
    ...
    clearHandler (all) {
      ...
      // }
      this.searchForm.payOrderId = this.searchForm.mchId = this.searchForm.mchOrderNo = this.searchForm.status = ''
      this.pageInfo.payOrderId = this.pageInfo.mchId = this.pageInfo.mchOrderNo = this.pageInfo.status = ''
    },
    clientInfo (id) {
      getPayInfo(id)
        .then(
