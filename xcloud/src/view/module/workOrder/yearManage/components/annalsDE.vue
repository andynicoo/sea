  ...
  <div class="container">
    <div class="taxBox">
      ...
      <div class="tittleStyle">
        <div class="talkbubble">提交申报（年报）</div>
      ...
      <div>
        <div class="container">
          ...
          <Row v-if="baseData.productFunctionCode == 2">
            ...
            <Col span="3">
              <DatePicker type="month" placeholder="开始时间" :options="options" style="width: 150px" v-model="kaishiTime" @on-change="kaishiTimeChange"></DatePicker>
            </Col>
            <Col span="1">
              <span class="tax-info-value" style="position: relative; top: 6px; left: 13px">{{ '至' }}</span>
            </Col>
            <Col span="3">
              <DatePicker type="month" style="width: 150px" :value="baseData.endTime" disabled></DatePicker>
            </Col>
          </Row>
      ...
      <div class="tittleStyle">
        <div class="talkbubble">申报数据</div>
        <div class="triangle"></div>
      </div>
      <div class="last-year-box">
        <p v-if="baseData.productFunctionCode == 2">
          请上传
          <span>{{ kaishiTimeMonth + '至' + baseData.endTime }}</span>
          的申报数据及文件；
        </p>
        <p v-else-if="baseData.productFunctionCode == 5">
          请上传
          <span>{{ baseData.time }}月</span>
          的申报数据及文件；
        </p>
        <p>温馨提示：</p>
        <div>
          ...
          <p>（若2022年5月才开始在我司申报，但税号生效日期是在2022年之前已经生效，2022年年报申报区间应填写2022-1-2022-12，并补充1-4月申报数据（上传申报回执和支付凭证，</p>
          <p>正确填写申报金额），若税号生效日是2022年5月，2022年年报区间应从税号生效月份开始，即2022-5-2022-12）；</p>
          <p>2.请正确选择申报周期、申报月份/季度；</p>
          <p>3.不含税销售额、应缴纳金额和抵扣金额应和申报回执上的金额一致，请仔细核对，如存在差异请将金额修改成申报回执上的金额</p>
          <p>4.应缴金额=申报回执上83项；</p>
          <p>实缴金额=缴税凭证上的金额；</p>
          <p>（填写提示：实缴金额包含当期产生的滞纳金，例如：3月税金100，滞纳金1， 客户实际缴纳101，此处填写101 按最终实际缴纳金额填写，未缴纳费用填写0）</p>
          <p>
          ...
          <p>6.平台最终会以您填写的年报数据提交至税局申报，如因个人原因导致需要修正申报，需另购买税务服务且后果自行承担；</p>
          <p>7. 如申报过程中有疑问请咨询在线客服；</p>
        </div>
      </div>

      <span class="red" v-if="baseData.productFunctionCode == 2">（注：请先选择申报区间再填写申报数据）</span>
      <Table :data="data" :columns="columns" border>
        <template slot="declareReceptSlot" slot-scope="{ row }">
          <div @click="getTableIndex(row, 'declareReceipt')">
            <Upload
              ...
              class="upload-box"
            >
              <Button icon="ios-cloud-upload-outline" class="amzUpload btnfile" style="margin-top: 10px" size="small"> 点击上传 </Button>
            </Upload>
            <div v-if="data[row._index].declareReceipt">
              <a :href="data[row._index].declareReceipt.slice(0, 5) == 'http:' ? data[row._index].declareReceipt.slice(0, 4) + 's' + data[row._index].declareReceipt.slice(4) : data[row._index].declareReceipt" target="_blank" style="font-size: 12px">
                {{ '申报回执' }}
              </a>
              <Button class="deletBtn" type="text" ghost @click="handleRemoveFile(1, 1, row, 'declareReceipt')">
                <img class="closeImg" src="@/assets/images/activeCloseImg.png" alt="" />
              </Button>
            </div>
          </div>
        </template>
        <template slot="payReceptSlot" slot-scope="{ row }">
          <div @click="getTableIndex(row, 'payReceipt')">
            ...
            <div v-if="data[row._index].payReceipt">
              <a :href="data[row._index].payReceipt.slice(0, 5) == 'http:' ? data[row._index].payReceipt.slice(0, 4) + 's' + data[row._index].payReceipt.slice(4) : data[row._index].payReceipt" target="_blank" style="font-size: 12px">
                {{ '缴费凭证' }}
              </a>
        ...
        <template slot="fillDiscount" slot-scope="{ row }">
          <div
            ...
            "
            style="cursor: pointer; color: rgba(22, 173, 233, 1)"
          >
            填写
          </div>
        </template>
      ...
      <div class="collect-box" :key="totalCollect">
        ...
        <div class="total-right">
          <div class="total-right-top">
            ...
            <span>进项税金累计：€ {{ returnTaxData.deductionPrice }}</span>
    ...
    <Modal v-model="discountBox" title="抵扣金额" :mask-closable="false">
      <Form :model="data[dataIndex]" :label-width="127">
        <FormItem label="发票抵扣">
          <Input v-model="data[dataIndex].invoiceDeductiontPrice" placeholder="请输入发票抵扣金额" style="width: 200px; margin-right: 10px"></Input>
          EUR（欧元）
        </FormItem>
        <FormItem label="C88抵扣">
          <Input v-model="data[dataIndex].c88DeductiontPrice" placeholder="请输入C88抵扣" style="width: 200px; margin-right: 10px"></Input>
          EUR（欧元）
        </FormItem>
        <FormItem label="B2B发票抵扣">
          <Input v-model="data[dataIndex].b2bDeductionPrice" placeholder="请输入B2B发票抵扣" style="width: 200px; margin-right: 10px"></Input>
          EUR（欧元）
        </FormItem>
        <FormItem label="其它抵扣">
          <Input v-model="data[dataIndex].euDeductiontPrice" placeholder="请输入其它抵扣" style="width: 200px; margin-right: 10px"></Input>
          EUR（欧元）
        </FormItem>
        <FormItem label="共享库存_采购">
          <Input v-model="data[dataIndex].comminglingBuySale" placeholder="请输入共享库存_采购" style="width: 200px; margin-right: 10px"></Input>
          EUR（欧元）
        </FormItem>
      </Form>
      <div slot="footer">
    ...
    <Modal v-model="submitModal" title="请确认您的申报数据" :width="616" class="submit-box">
      ...
      <div class="submit-tip-box">
        <div class="reportData-box">
          ...
          <div class="sureData-box">
            ...
            <div class="row">
              <span>Vorsteuer（欧盟内免税收购项(递延清关)累计）</span>
      ...
      <div slot="footer">
        <Button @click="submitModal = false">取消</Button>
        <Button type="primary" @click="comfireSubmit">确认</Button>
      </div>
    </Modal>
    ...
    <Modal v-model="showImg" width="1200">
      <img style="width: 1140px" src="https://file.itaxs.com/pro/20230203/5b2b892fdf4b4ee4a068b2e61c56026e.png" alt="" />
    </Modal>
  </div>
</template>
<script>
import * as API from '@/api/taxService/annalsDE.js';
import { getOrderYearListAPI, OrderYearDetailAPI, yearSubmitAPI, deAnnalsSureSubmitApi } from '@/api/newApi/workOrder/annualReport';
...
export default {
  data() {
    return {
      ...
      baseData: JSON.parse(this.$route.query.baseDataDe),
      ...
      options: {
        disabledDate(date) {
          const { endTime } = JSON.parse(this.$route.query.baseDataDe);
      ...
      data: [
        {
          ...
          taxRate: 19,
          paidTaxPrice: 0, //实缴金额
          otherEcAcqPrice: 0, //欧盟内免税收购项金额（原采购金额））
          agentAmt: 0, //平台代扣代缴
              ...
              h('Input', {
                ...
                on: {
                  input: (val) => {
                    ...
                    this.data.map((item) => {
                      this.returnTaxData.otherEcAcqPrice = this.printFn(Number(item.otherEcAcqPrice) + this.returnTaxData.otherEcAcqPrice);
                    });
                    this.totalCollect = new Date().getTime();
                  },
                },
  ...
  mounted() {
    ...
    // debugger
    // console.log(this.baseData, 222);
    // console.log(this.baseData.vatTaxNumber, 222);
  ...
  methods: {
    ...
    remove(index) {
      ...
      this.data.map((item) => {
        ...
        this.returnTaxData.deductionPrice = this.printFn(Number(item.b2bDeductionPrice) + this.returnTaxData.deductionPrice);
        this.returnTaxData.deductionPrice = this.printFn(Number(item.euDeductiontPrice) + this.returnTaxData.deductionPrice);
        this.returnTaxData.deductionPrice = this.printFn(Number(item.comminglingBuySale) * 0.19 + this.returnTaxData.deductionPrice);
        this.returnTaxData.allTaxPrice = this.printFn(Number(item.allTaxPrice) + this.returnTaxData.allTaxPrice);
        this.returnTaxData.excludeTaxPrice = this.printFn(Number(item.excludeTaxPrice) + this.returnTaxData.excludeTaxPrice);
        this.returnTaxData.paidTaxPrice = this.printFn(Number(item.paidTaxPrice) + this.returnTaxData.paidTaxPrice);
        // console.log("this.returnTaxData.paidTaxPrice", this.returnTaxData.paidTaxPrice);
        this.returnTaxData.otherEcAcqPrice = this.printFn(Number(item.otherEcAcqPrice) + this.returnTaxData.otherEcAcqPrice);
        this.returnTaxData.agentAmt = this.printFn(Number(item.agentAmt) + this.returnTaxData.agentAmt);
      });
      this.returnTaxData.deductionPrice = this.$util.formatPrice(this.returnTaxData.deductionPrice);
      ...
      OrderYearDetailAPI(data).then((res) => {
        if (res.code === 0) {
          if (res.data.length > 0) {
            ...
            this.data = res.data;
            this.initAggregateData(res.data);
    ...
    submitModalFun() {
      ...
      data.beginTime = this.translateDate(data.beginTime, 'm');
      yearSubmitAPI(data).then((res) => {
        if (res.code == 0) {
          this.wenxinTip = false;
          this.returnTaxData = Object.assign(this.returnTaxData, res.data);
    ...
    comfireSubmit() {
      ...
      deAnnalsSureSubmitApi(data).then((res) => {
        if (res.code == 0) {
    ...
    getDEyearDetailFun(row) {
      ...
      getOrderYearListAPI(data).then((res) => {
        if (res.code === 0) {
