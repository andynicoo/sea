      ...
      <div class="search-box" :class="moreSerch ? 'styleclass' : ''">
        <el-form ref="searchForm" :model="form" inline label-width="100px" @keydown.enter.native="search">
          ...
          <el-form-item style="float: right">
            <div class="header-btn-group" v-show="hasAuthority('K2_7')">
...
import { eprAgingList, erpReportExport } from '@/api/newApi/workOrder/dataCenter.js';
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      eprAgingList(this.getParams()).then((res) => {
        this.cellDataObj = res.data || {};
        this.list = res.data.reportEPRDeliveryTimeListVos || [];
    ...
    exportFile() {
      ...
      erpReportExport(this.getParams())
        .then((res) => {
          this.exportLoading = false;
          this.$util.exportExcel(res, '合规交付时效');
        })
        .catch((e) => {
          this.exportLoading = false;
        });
