  ...
  <div class="app-container">
      ...
      <div class="search-box" :class="moreSerch ? 'styleclass' : ''">
        <el-form ref="searchForm" :model="form" inline label-width="100px" @keydown.enter.native="search">
          ...
          <el-form-item style="float: right">
            <div class="header-btn-group" v-show="hasAuthority('K2_6')">
    ...
    <div class="table-container">
      <div class="cell-wrap">
        ...
        <div class="cell">
          <div class="tit">律师审核工单数</div>
          <div class="num">{{ cellDataObj.attorneyNum || 0 }}</div>
          <div class="flex">
            <div class="li">人效: {{ cellDataObj.attorneyPersonEffect || 0 }}</div>
            <div class="li">时效: {{ cellDataObj.lawyersTimeTest || 0 }}</div>
          </div>
        </div>
      </div>
      ...
      >
        ...
        <el-table-column
          ...
          sort-by="firstTime"
        />
        <el-table-column align="center" label="律师审核工单数" show-overflow-tooltip prop="attorneyNum" sortable />
        <el-table-column
          align="center"
          label="律师审核时效"
          show-overflow-tooltip
          prop="lawyersTimeTest"
          sortable
          sort-by="lawyersTime"
        />
      </el-table>
...
import { getTrademarkAgingAll, getTrademarkAgingList, getTrademarkExport } from '@/api/newApi/workOrder/dataCenter.js';
...
export default {
  ...
  methods: {
    ...
    search() {
      this.getList();
      this.getTrademarkAgingAll();
    },
    // 获取列表
    getList() {
      this.tableLoading = true;
      getTrademarkAgingList(this.getParams()).then((res) => {
        this.list = res.data || [];
    ...
    exportFile() {
      ...
      getTrademarkExport(this.getParams())
        .then((res) => {
          this.exportLoading = false;
          this.$util.exportExcel(res, '商标交付时效');
        })
        .catch((e) => {
          this.exportLoading = false;
        });
    ...
    // 获取数据
    getTrademarkAgingAll() {
      getTrademarkAgingAll(this.getParams()).then((res) => {
