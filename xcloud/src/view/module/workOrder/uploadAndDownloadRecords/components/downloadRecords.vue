  ...
  <div class="app-container">
      ...
      <div class="search-box">
        <el-form ref="searchForm" :model="form" inline label-width="70px">
          ...
          <div class="header-btn-group" style="float: right">
            <el-button type="primary" @click="delRecord(ids)" :disabled="!ids.length">删除</el-button>
          </div>
        </el-form>
      </div>
    </div>
    ...
    <div class="table-container">
      <el-table border :height="tableHeight" :data="list" v-loading="tableLoading" @selection-change="handleSelectionChange" ref="table" element-loading-spinner="icon loading" element-loading-text="拼命加载中">
        <Empty slot="empty" />
        <el-table-column type="selection" :selectable="selectable" align="center" fixed="left" width="55"></el-table-column>
        <el-table-column align="center" min-width="150" label="下载内容" show-overflow-tooltip prop="description"> </el-table-column>
        <el-table-column align="center" width="150" label="下载时间" show-overflow-tooltip prop="createTime"></el-table-column>
        <el-table-column align="center" min-width="100" label="下载状态" show-overflow-tooltip prop="status">
        ...
        <el-table-column align="center" width="200" label="操作" fixed="right">
          <template slot-scope="{ row }">
            <el-link type="primary" :underline="false" @click="exportRecord(row.fileUrl)" :disabled="row.status !== 2"> 导出 </el-link>
            <el-link type="primary" :underline="false" @click="delRecord([row.id])" :disabled="row.status === 1">删除</el-link>
          </template>
        </el-table-column>
      </el-table>
    </div>
...
export default {
  ...
  data() {
    return {
      ...
      copyListQuery: {},
      statusList: [
        { label: '下载失败', value: 3, icon: 'error' },
        { label: '下载成功', value: 2, icon: 'success' },
        { label: '下载中', value: 1, icon: 'loading' },
      ],
      tableLoading: false,
      searchFlag: false,
      ids: [],
