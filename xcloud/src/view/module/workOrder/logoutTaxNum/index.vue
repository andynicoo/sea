      ...
      <template #operator>
        <el-dropdown @command="command" style="margin-left: 20px">
          ...
          <el-dropdown-menu slot="dropdown">
            <el-dropdown-item command="changeManager" v-show="hasAuthority('P1_1')">修改交付专员</el-dropdown-item>
            <el-dropdown-item command="batchChangeCustom" v-show="hasAuthority('P1_12')">修改客服</el-dropdown-item>
          </el-dropdown-menu>
        </el-dropdown>
      </template>
    ...
    <WorkOrderTable ref="workOrderTable" :form="form" :method="method" :functionCode="functionCode" @handleSelectionChange="handleSelectionChange">
      <template #operator="{ row }">
        <el-tooltip content="查看" placement="top">
          <el-link type="primary" :underline="false" @click="detail(row)" v-show="hasAuthority('P1_3')">
            <i class="icon icon-see"></i>
          </el-link>
        </el-tooltip>
        <el-tooltip content="添加备注" placement="top">
          <el-link type="primary" :underline="false" @click="addRemark(row)" v-show="hasAuthority('P1_4')">
...
import { getTaxCancelList } from '@/api/newApi/workOrder/logoutTaxNum';
