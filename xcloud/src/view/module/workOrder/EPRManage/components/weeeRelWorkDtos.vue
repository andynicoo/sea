        ...
        <el-table-column :label="(isEPRTaxServices ? '转代理' : '注册') + '产品信息'" align="center">
          ...
          <el-table-column min-width="150px" align="center" label="* 生效月份" prop="effectDate">
            <template slot-scope="scope">
              <el-form-item :prop="'weeeInfos.' + scope.$index + '.effectDate'" :rules="baseRule">
                <el-date-picker v-model="scope.row.effectDate" type="month" value-format="yyyy-MM-dd HH:mm:ss" placeholder="选择月"> </el-date-picker>
              </el-form-item>
            </template>
          </el-table-column>
          ...
          <el-table-column align="center" label="操作" width="100px" fixed="right" v-if="!showOnly">
            <template slot-scope="scope">
              ...
              <el-link type="primary" @click="delItem(scope.$index)" :disabled="disabled" v-if="form.weeeInfos.length > 1"> 删除 </el-link>
            </template>
          </el-table-column>
        </el-table-column>
      </el-table>
