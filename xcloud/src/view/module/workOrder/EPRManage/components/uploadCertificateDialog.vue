<template>
  <el-dialog :title="isReplace ? '替换协议证书' : '替换授权书'" :visible.sync="dialogvisible" width="600px" center @close="reset">
...
import { submitEpr, saveEpr, replaceAuthCertificate } from '@/api/newApi/workOrder/EPRManage';
export default {
  props: {
    ...
    formSave() {
      this.$refs.dynamicForm.onSubmit().then((materialData) => {
        ...
        if (this.isReplace) {
          ...
          replaceAuthCertificate(data).then((res) => {
            if (res.code === 0) {
              this.dialogvisible = false;
              this.$message.success('提交成功');
              this.$emit('refresh');
