<template>
  <el-dialog title="录入注册结果" :visible.sync="dialogvisible" width="660px" center @close="reset">
    <el-form :model="form" ref="form" :rules="computRule" label-width="110px">
      ...
      <template v-if="nextNodeCode == 'NODE0365'">
        <p class="mb-15">
...
import { inputEprRegisterInfoApi, readBankInfoApi } from '@/api/newApi/workOrder/EPRManage';
export default {
  props: {
  ...
  watch: {
    dialogvisible(newVal) {
      if (newVal) {
        ...
        if (this.eprWorkOrder.bankPayInfos) {
          try {
  ...
  methods: {
    ...
    submit() {
      ...
      this.$refs['form'].validate((valid) => {
        if (valid) {
          if (this.form.officialFee == 0) {
            return this.$message.error('官费请输入大于0的数');
          }
