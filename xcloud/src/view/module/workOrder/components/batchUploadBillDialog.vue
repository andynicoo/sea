  ...
  <el-dialog :visible.sync="dialogvisible" width="650px" center @close="reset">
    ...
    <template>
      <el-form :model="form" :rules="rules" ref="form">
        <h3 class="step-title">步骤1：准备批量上传的账单信息</h3>
        <p>
          <span>下载模板，按照模版填写信息</span>
        </p>
        <p>
          <el-button @click="downloadTemplate">下载账单信息模版</el-button>
        </p>
        ...
        <p>不得修改信息表命名；</p>
        <p>表格内所有字段必须是文本格式；</p>
        <h3 class="step-title">步骤2：准备批量上传的账单文件</h3>
        <p>
          <span>每个账单文件按要求命名</span>
        </p>
        <p>
          <span>账单文件命名格式：</span>
        ...
        <p>
          <span>将编辑好的信息表和账单文件全选，生成1个压缩文件包上传</span>
        </p>
        <p class="text-danger">注意：</p>
        <p>压缩包仅限zip格式；</p>
        <p>压缩包内不能有文件夹；</p>
        ...
        <el-form-item prop="doNext">
          <el-checkbox v-model="form.doNext">上传成功后自动流转工单状态</el-checkbox>
        </el-form-item>
      </el-form>
...
export default {
  props: {
    batchUploadBillDialog: Boolean,
    checkIds: Array,
  },
  computed: {
    dialogvisible: {
      get() {
        return this.batchUploadBillDialog;
      },
      set(val) {
        this.$emit('update:batchUploadBillDialog', val);
      },
    },
  },
  components: {
    ...
    submit() {
      this.$refs.form.validate((valid) => {
        if (valid) {
          ...
          this.$util
            .formDataRequest({
              url: '/workorder/management/vat/epr/bill/batch',
              data: formData,
              params: {
                doNext: this.form.doNext,
              },
            })
            .then((res) => {
