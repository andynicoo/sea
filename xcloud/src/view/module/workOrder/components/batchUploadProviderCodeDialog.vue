  ...
  <el-dialog :visible.sync="dialogvisible" width="650px" center @close="reset">
    ...
    <template>
      <el-form :model="form" :rules="rules" ref="form">
        <h3 class="step-title">步骤1：准备批量上传的服务商分配编码信息</h3>
        <p>
        ...
        <p>表格内所有字段必须是文本格式</p>
        <p>
          <el-button>下载服务商分配编码信息模版</el-button>
        </p>
        <h3 class="step-title">步骤2：上传编辑好的信息表</h3>
        <p>
          <span>将编辑好的服务商分配编码信息表上传；</span>
        </p>
        <p class="text-danger">注意：</p>
        <p>文件大小&lt;100M；</p>
...
export default {
  props: {
    batchUploadProviderCodeDialog: Boolean,
    checkIds: Array,
  },
  computed: {
    dialogvisible: {
      get() {
        return this.batchUploadProviderCodeDialog;
      },
      set(val) {
        this.$emit('update:batchUploadProviderCodeDialog', val);
      },
    },
  },
  components: {
