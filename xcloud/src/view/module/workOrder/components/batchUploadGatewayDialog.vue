  ...
  <el-dialog :visible.sync="dialogvisible" width="650px" center @close="reset">
    ...
    <template>
      <el-form :model="form" :rules="rules" ref="form">
        <h3 class="step-title">步骤1：准备批量上传的gateway账号密码</h3>
        <p>
        ...
        <p>表格内所有字段必须是文本格式；</p>
        <p>
          <el-button @click="downloadTemplate">下载gateway账号密码信息模版</el-button>
        </p>
        <h3 class="step-title">步骤2：上传编辑好的信息表</h3>
        <p>
          <span>将编辑好的信息表上传；</span>
        </p>
        <p class="text-danger">注意：</p>
        <p>文件后缀名必须为：xls或xlsx（Excel标准格式），文件大小&lt;100M；</p>
        <p>表格信息中如果已有重复数据，系统将自动覆盖历史数据信息；</p>
        <el-form-item prop="fileList">
          <ManualUploadFile v-model="form.fileList" accept=".xls,.xlsx" />
        </el-form-item>
      </el-form>
...
export default {
  props: {
    batchUploadGatewayDialog: Boolean,
    checkIds: Array,
  },
  computed: {
    dialogvisible: {
      get() {
        return this.batchUploadGatewayDialog;
      },
      set(val) {
        this.$emit('update:batchUploadGatewayDialog', val);
      },
    },
  },
  components: {
  ...
  methods: {
    downloadTemplate() {
      window.open(this.baseUrl + `/workorder/management/vat/registerTaxInfo/gatewayTemplate`);
    },
    submit() {
      this.$refs.form.validate((valid) => {
        if (valid) {
          ...
          this.$util
            .formDataRequest({
              url: '/workorder/management/vat/registerTaxInfo/batch/gateway',
              data: formData,
            })
            .then((res) => {
