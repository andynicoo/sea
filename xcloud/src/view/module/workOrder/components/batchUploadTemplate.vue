<template>
  <el-dialog :visible.sync="dialogvisible" width="650px" center @close="reset">
    ...
    <template>
      <el-form :model="form" :rules="rules" ref="form" class="upload-dialog">
        <h3 class="step-title">步骤1：准备需要上传的信息</h3>
        <p>
        ...
        <p>表格内所有字段必须是文本格式；</p>
        <p>
          <el-button @click="downloadTemplate">下载信息模版</el-button>
        </p>
        <slot name="step2">
        ...
        <el-form-item prop="fileList">
          <ManualUploadFile v-model="form.fileList" :accept="accept ? accept : '.xls,.xlsx'" />
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
    downloadTemplate() {
      // window.open(this.baseUrl + `/workorder/management/vat/epr/uinTemplate`);
      window.open(this.baseUrl + this.templateUrl);
    },
    submit() {
      this.$refs.form.validate((valid) => {
        if (valid) {
          ...
          // this.$util
          //   .formDataRequest({
          //     url: '/workorder/management/vat/epr/uin/batch',
          //     data: formData,
          //   })
          // return;
          this.$util
            .formDataRequest({ url: this.submitUrl, data: formData })
            .then((res) => {
