  ...
  <div class="app-container">
      ...
      <div class="search-box" :class="moreSerch ? 'styleclass' : ''">
        <el-form ref="searchForm" :model="form" inline label-width="100px">
          ...
          <el-form-item style="margin-left: 20px">
            ...
            <el-dropdown @command="(c) => this[c]()" style="margin-left: 20px">
              ...
              <el-dropdown-menu slot="dropdown">
                <el-dropdown-item command="batchRemind" v-show="(tabNav === progressBarObj.WAIT_UPLOAD || tabNav === progressBarObj.WAIT_AUTHORIZE) && hasAuthority('X1_1')">批量提醒</el-dropdown-item>
                <el-dropdown-item command="changeManager" v-show="hasAuthority('X1_2')">修改交付专员</el-dropdown-item>
                <el-dropdown-item command="batchChangeCustom" v-show="hasAuthority('X1_20')">修改客服</el-dropdown-item>
                <el-dropdown-item command="_export" v-show="hasAuthority('X1_3')">导出</el-dropdown-item>
              </el-dropdown-menu>
            </el-dropdown>
          </el-form-item>
          ...
          <div v-show="moreSerch">
            ...
            <el-form-item label="服务进度" prop="currentNodeCode">
    ...
    <div class="table-container">
      ...
      <el-table border :data="list" v-loading="tableLoading" element-loading-spinner="icon loading" element-loading-text="拼命加载中" @selection-change="handleSelectionChange" :height="tableHeight">
        ...
        <el-table-column align="center" width="150" label="服务类型" show-overflow-tooltip prop="serviceSpecName"></el-table-column>
        <el-table-column align="center" width="200px" label="注册主体名称" show-overflow-tooltip prop="companyNameZh"></el-table-column>
        ...
        <el-table-column align="center" width="150" label="服务商" show-overflow-tooltip prop="supplierName">
          <span class="show-tooltip-wrapper" slot-scope="scope">
            <el-link type="primary" class="el-icon-edit show-tooltip-icon" @click="changeServiceProvider(scope.row)" v-if="hasAuthority('X1_4') && (scope.row.nodeCode === progressBarObj.WAIT_UPLOAD || scope.row.nodeCode === progressBarObj.WAIT_REVIEW || !scope.row.supplierId)"></el-link>
            <span class="show-tooltip">{{ scope.row.supplierName }}</span>
          </span>
        </el-table-column>
        <el-table-column align="center" width="100" label="驳回详情" show-overflow-tooltip prop="sender">
          <template slot-scope="scope">
            <el-link v-if="hasAuthority('X1_5') && scope.row.existReject" @click="checkRejectInfo(scope.row)" type="primary"><i class="icon icon-search-small"></i>查看</el-link>
            <span v-else>-</span>
          </template>
        </el-table-column>
        <el-table-column align="center" width="100" label="订单备注" show-overflow-tooltip prop="orderRemark">
          <template slot-scope="scope">
            <el-link
              v-if="hasAuthority('X1_19') && scope.row.orderRemark"
              @click="
        ...
        <el-table-column align="center" width="150" label="创建时间" sortable show-overflow-tooltip prop="createTime"></el-table-column>
        <el-table-column align="center" width="150" label="审核通过时间" sortable show-overflow-tooltip prop="checkPassTime">
          <template slot-scope="scope">{{ scope.row.checkPassTime || '-' }}</template>
        </el-table-column>
        ...
        <el-table-column align="center" width="70" label="操作" fixed="right">
          <template slot-scope="scope">
            <el-tooltip content="查看" placement="top">
              <el-link type="primary" :underline="false" @click="detail(scope.row)" v-show="hasAuthority('X1_6')">
    ...
    <BatchDownloadDialog :batchDownloadDialog.sync="batchDownloadDialog" contentText="英代/欧代资料（包含授权书）" :checkIds="checkIds" />
...
import { getAuthAgentList, authAgentExport, authAgentNotice } from '@/api/newApi/workOrder/authorizedRepresent';
import { serviceNode } from '@/api/newApi/taxServices/serviceManage.js';
import cacheQuery from '@/mixins/cacheQuery';
import { getUserListByRole, serviceProgress, getSupplierList } from '@/api/newApi/common';
...
export default {
  ...
  data() {
    return {
      ...
      countrySelectList: [
        ...
        { countryCode: 'US', countryNameZh: '美国' },
      ],
      supplierList: [],
      checkItem: {},
      tableLoading: false,
  ...
  methods: {
    ...
    getList() {
      ...
      getAuthAgentList(formData)
        .then((res) => {
          ...
          this.tabNav = this.form.currentNodeCode === '' ? '-1' : this.form.currentNodeCode;
    ...
    getServiceNode() {
      serviceNode({ functionCode: this.$constant.AUTHORIZEDREPRESENT_FUNCTION_CODE })
        .then((res) => {
          if (res.code == 0) {
            this.tabs = res.data;
          }
    ...
    batchRemind() {
      ...
      this.$confirm(`已选中${this.checkIds.length}个客户，确定是否给选中客户发送短信提醒？`, '提示', {
        ...
        authAgentNotice({
          nodeCode: this.tabNav,
          serviceIds: this.checkIds,
        }).then((res) => {
          if (res.code === 0) {
