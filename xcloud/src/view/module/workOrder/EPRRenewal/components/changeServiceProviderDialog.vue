      ...
      <el-form-item prop="supplierId">
        <el-select style="width: 410px" v-model="form.supplierId" size="small" filterable>
          <el-option-group>
            ...
            <el-option v-for="item in rowSupplierList" :key="item.id" :label="item.shortName" :value="item.id" class="option_title">
    ...
    submit() {
      this.$refs['form'].validate((valid) => {
        if (valid) {
          ...
          this.$emit('changeProvider', this.form);
        }
      });
    },
    // confirm() {
