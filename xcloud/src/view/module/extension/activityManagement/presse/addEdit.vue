  ...
  <div class="app-container">
    ...
    <div class="form-container">
      <el-form label-width="140px" ref="formRef" :model="form" :rules="formRules">
        ...
        <FormTitle value="活动设置："></FormTitle>
        <el-form-item label="购买方式：">
          <el-radio-group v-model="form.buyMethod" :disabled="isEdit">
            <el-radio label="LOWEST_PRICE">最低价可购买</el-radio>
            <el-radio label="RANDOM_PRICE">任意金额可购买</el-radio>
          </el-radio-group>
        </el-form-item>
        <el-form-item label="砍价有效期：" prop="bargainEffectiveHour">
          <el-input-number v-model="form.bargainEffectiveHour" controls-position="right" placeholder="请输入" style="width: 130px; margin-right: 10px" clearable :min="0" :max="999" :disabled="isEdit" />
          <span>小时</span>
          <div>用户发起砍价到砍价截止的时长，超出时间没有达到帮砍人数视为砍价失败</div>
        </el-form-item>
        <el-form-item label="每刀金额：">
          <el-radio-group v-model="form.bargainAmountType" :disabled="isEdit">
            <el-radio label="RANDOM">每刀任意金额</el-radio>
            <el-radio label="SECURE">每刀固定金额</el-radio>
          </el-radio-group>
          <div>选择每刀固定金额时，每人砍价金额相同，达到帮砍人数才可砍至底</div>
        </el-form-item>
        <el-form-item label="每人发起次数：" prop="timeLimit">
          <el-input-number v-model="form.timeLimit" controls-position="right" placeholder="请输入" style="width: 130px; margin-right: 10px" clearable :min="0" :max="999" :disabled="isEdit" />
          <span>次</span>
          <div>活动期间内，同一个活动商品允许用户发起的砍价次数</div>
        </el-form-item>
        <el-form-item>
          <el-button type="primary" :loading="submitting" @click="handleSubmit" v-show="hasAuthority('E2_13')" :disabled="isEdit && status != 'PROCESSING'"> 保存 </el-button>
