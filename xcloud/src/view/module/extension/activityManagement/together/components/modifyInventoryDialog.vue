...
<script>
import { updateTogtherStockApi } from '@/api/newApi/extension/activity.js';
export default {
  props: {
  ...
  methods: {
    submit() {
      this.$refs['form'].validate((valid) => {
        if (valid) {
          let data = {
            productSpecsId: this.rowData.productSpecsId,
            spellGroupActivityId: this.rowData.spellGroupActivityId,
            num: this.form.num,
            productInfoId: this.rowData.productInfoId,
          };
          updateTogtherStockApi(data)
            .then((res) => {
              if (res.code === 0) {
