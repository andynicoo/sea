    ...
    <div class="header-container" ref="searchBox">
      <el-form ref="searchForm" inline label-width="70px">
        ...
        <el-form-item style="float: right">
          <el-button type="success" @click="goProductDetail('add')" icon="el-icon-plus"> 新建定金膨胀活动 </el-button>
        </el-form-item>
      </el-form>
    </div>
    <!-- 列表 -->
    <div class="table-container">
      <el-table :data="dataSource" border :height="tableHeight" ref="table" v-loading="listLoading" element-loading-spinner="icon loading" element-loading-text="拼命加载中">
        ...
        <el-table-column align="center" label="活动时间" sortable prop="supplierId" width="300px">
          <template slot-scope="scope">{{ scope.row.depositPaymentStartTime }}至 {{ scope.row.depositPaymentFinishTime }}</template>
        </el-table-column>
        ...
        <el-table-column align="center" label="定金支付人数">
          <template slot-scope="scope">{{ scope.row.depositPaymentCount }}</template>
        </el-table-column>
        <el-table-column align="center" label="支付尾款人数">
          <template slot-scope="scope">{{ scope.row.finalPaymentCount }}</template>
        </el-table-column>
...
<script>
import { preSaleActivityList, depositDelete, depositUpperOrLower } from '@/api/newApi/extension/activity.js';
export default {
  data() {
    return {
      dataSource: [],
  ...
  methods: {
    ...
    queryList() {
      ...
      preSaleActivityList(this.listQuery)
        .then((res) => {
          if (res.code === 0) {
    ...
    toDelete(row) {
      this.$confirm('是否确定删除活动？', '提示', {
        ...
        .then(() => {
          depositDelete({ id: row.preSaleActivityId })
            .then((res) => {
              if (res.code === 0) {
    ...
    goProductDetail(type) {
      this.$router.push({ path: '/extension/activityManagement/deposit/addEdit', query: { key: type } });
    },
  },
};
</script>
