    ...
    <Card v-if="!isDetail">
      ...
      <Table border
        ...
        <template slot="operate"
          ...
          <Button type="primary"
                  ghost
                  @click="toDetail(row.userId,row.mobile)">查看</Button>
          <Button type="primary"
                  ghost
                  v-if="row.followUserId === '0'"
                  @click="toDoClaim(row.userId)">认领</Button>
        </template>
      </Table>
    </Card>
    <Modal v-model="claim"
...
<script>
import { changeUserStatus, searchList, claim } from '@/api/iipto/user'
