<template>
  <div>
    ...
    <Form :model="addFrom"
      ...
      <FormItem label="文章标题："
                prop="bulletinTitle">
        <Input v-model="addFrom.bulletinTitle"
               :maxlength="30"
               placeholder="请输入文章标题"></Input>
      </FormItem>
      <FormItem label="摘要："
                prop="bulletinSummary">
        <Input v-model="addFrom.bulletinSummary"
               :maxlength="70"
               type="textarea"
               placeholder="请输入摘要"></Input>
      </FormItem>
      <FormItem label="资讯分类："
                prop="bulletinType">
        <Select v-model="addFrom.bulletinType"
                placeholder="选择资讯分类">
          <Option :value="item.type"
                  v-for="item in typeList"
                  :key="item.type">{{item.name}}</Option>
        </Select>
      </FormItem>
      <FormItem label="主图："
                prop="bulletinMainPicture">
        <Upload type="drag"
      ...
      <FormItem label="广告链接："
                prop="bulletinAdPictureUrl">
        <Input v-model="addFrom.bulletinAdPictureUrl"
               placeholder="广告链接："
               style="width:400px;"></Input>
      </FormItem>
      <FormItem label="内容："
                prop="bulletinContent">
        <vue-neditor-wrap v-model="addFrom.bulletinContent"
                          :config="myConfig"
                          class="editbd"
                          :destroy="false"></vue-neditor-wrap>
      </FormItem>
      <FormItem label="作者："
                prop="bulletinAuthorName">
        <Input v-model="addFrom.bulletinAuthorName"
               placeholder="请输入作者名称"
               style="width:400px;"></Input>
      </FormItem>
      <FormItem label="是否启用："
                prop="bulletinStatus">
        <i-switch v-model="addFrom.bulletinStatus"
...
<script>
import { submitInformation } from '@/api/iipto/information.js'
export default {
  props: ['detail'],
  data() {
    return {
