<template>
  <div>
    ...
    <Card style="margin-top:12px;">
      <p class="title">录入证据</p>
      <Form :model="formInfo" label-position="top" :rules="ruleValidate" ref="formInfo">
        <div class="step1-box" v-if="steps==1">
          <div v-if="!isEditor">
            <p class="info-title">商标信息</p>
            <FormItem label="商标类型" prop="trademarkType">
              <Select v-model="formInfo.trademarkType" placeholder="请选择商标类型" :disabled="isEditor">
                <Option value="1">纯文字</Option>
                <Option value="2">纯图形</Option>
                <Option value="3">文字+图形</Option>
              </Select>
            </FormItem>
            <FormItem label="商标名称（英文）" prop="trademarkNameEn" v-if="formInfo.trademarkType==1||formInfo.trademarkType==3">
              <Input v-model="formInfo.trademarkNameEn" placeholder="示例：ABC" :maxlength="200" :disabled="isEditor"></Input>
              <p class="gary-tip">最多200字符。</p>
            </FormItem>
            <FormItem label="商标图形LOGO" prop="trademarkLogo" v-if="formInfo.trademarkType==2||formInfo.trademarkType==3">
              <div class="bread-logo-box" :class="formInfo.trademarkLogo==''?'':'show-change'">
                <Upload :action="workOrderFile" v-if="formInfo.trademarkLogo==''" class="upload-logo" :on-format-error="formatErrorHandler1" :on-success="(res,file,fileList)=>{successHandler1(res,file,fileList,'trademarkLogo')}" :on-remove="(file,fileList)=>{removeHandler1 (file,fileList,'trademarkLogo')}" :format="['jpg','png','jpeg','svg']" :show-upload-list="false" :disabled="isEditor">
                  <img src="@/assets/images/infoDetail/upload.png" alt="">
                </Upload>
                <img :src="formInfo.trademarkLogo" class="bread-logo" alt="" v-else>
                <Upload class="hover-box" :on-format-error="formatErrorHandler1" :on-success="(res,file,fileList)=>{successHandler1(res,file,fileList,'trademarkLogo')}" :on-remove="(file,fileList)=>{removeHandler1 (file,fileList,'trademarkLogo')}" :format="['jpg','png','jpeg','svg']" :show-upload-list="false" :disabled="isEditor" :action="workOrderFile">
                  <img src="@/assets/images/infoDetail/editor-bai.png" alt="">更换图片
                </Upload>
              </div>
              <a class="example-box" v-if="formInfo.trademarkLogo!=''" :href="formInfo.trademarkLogo" target="_blank">
                查看原图
            ...
            <FormItem label="设计商标描述（如申请的商标含logo，则要填写设计商标描述）" prop="trademarkDescribe" class="width100" v-if="formInfo.trademarkType==2||formInfo.trademarkType==3">
              <Input v-model="formInfo.trademarkDescribe" type="textarea" :disabled="isEditor" :maxlength="500" style="width:398px;"></Input>
            </FormItem>
            <p class="info-title">将申请的商标分类</p>
            <div class="form-item">
              <span class="item-label red-tip">商标分类</span>
              <span class="item-upload" v-if="!isChooseOneClassify" @click="classifyModal=true">
                <img src="@/assets/images/infoDetail/jia.png" alt="">添加商标分类
              </span>
              <span v-if="isChooseOneClassify" class="classify-box">
                <img src="@/assets/images/infoDetail/editor.png" alt="" v-if="!isEditor" @click="classifyModal=true">
                <span v-for="(item,index) in choiceClassifyList" :key="index">{{item.categoryNum}}&nbsp;{{item.categoryName}}</span>
              </span>
              <p class="tip" v-if="!isChooseOneClassify">请选择商标分类</p>
            </div>
            <div class="form-item">
              <div>
                <span class="item-label red-tip from">商标小类名称（英文）</span>
              </div>
              <Input v-model="model13" :disabled="isEditor" placeholder="输入的单词必须是有含义的" style="width:398px;" :maxlength="200"></Input>
              <Button class="search-subclass-btn" @click="searchSubclass">搜索</Button>
              <Table border v-if="model13!=''||twoClassifyList.length>0" class="subclass-table" :data="twoClassifyList" :columns="subClassCloumns" @on-selection-change="selectData" @on-select-all-cancel="cancelData">
              </Table>
              <div class="check-two-category" v-if="isEditor">
                <span v-for="(item,index) in checkTwoClassifyList" :key="index">{{item.categoryNameEn}}</span>
              </div>
            </div>
            <div class="risk-box" v-if="riskClassifyList.length>0">
          ...
          <div class="form-item">
            <FormItem label="首次使用日期" prop="firstUseDate" class="first-date">
              <DatePicker type="date" :disabled="!isEditor" v-model="formInfo.firstUseDate" :options="options" placeholder="年/月/日格式，示例：2020/01/01"></DatePicker>
              <div class="first-tip">
                1. 两个日期可以相同，
            ...
            <FormItem label="首次在商业上使用的日期" prop="firstBusinessUseDate">
              <DatePicker type="date" :disabled="!isEditor" v-model="formInfo.firstBusinessUseDate" :options="options" placeholder="年/月/日格式，示例：2020/01/01"></DatePicker>
            </FormItem>
            <FormItem label="销售链接" class="width100 red-tip from" style="width:100%;">
              <div class="marketHref-list" v-for="(item,index) in marketHrefList" :key="index">
                <Input v-model="item.marketHref" :disabled="!isEditor" placeholder="示例：http://或https://" :maxlength="300" style="width:234px;"></Input>
                <Select style="width:156px;margin-left:10px;" placeholder="请选择商标小类" v-model="item.category2NameEn" @on-change="changeHrefClass($event,index)">
                  <Option v-for="(v,i) in checkTwoClassifyList" :key="i" :value="v.categoryNameEn">{{v.categoryNameEn}}</Option>
                </Select>
                <img src="@/assets/images/infoDetail/add.svg" alt="" v-if="index==(marketHrefList.length-1)&&marketHrefList.length<20" @click="addHref">
          ...
          <div class="proof-box form-item">
            <Row>
              ...
              <Upload class="upload-proof" ref="uploadEvidence" :before-upload="beforeUploadHandler" :on-format-error="formatErrorHandler" :on-success="(res,file,fileList)=>{successHandler(res,file,fileList,usrProductImgList)}" :action="workOrderFile" :show-upload-list="false" :disabled="!isEditor" :format="['jpg','pdf']">
              ...
              <Upload class="upload-proof" ref="uploadEvidence" :before-upload="beforeUploadHandler" :on-format-error="formatErrorHandler" :on-success="(res,file,fileList)=>{successHandler(res,file,fileList,useEvidenceList)}" :action="workOrderFile" :show-upload-list="false" :disabled="!isEditor" :format="['jpg','pdf']">
                <img src="@/assets/images/infoDetail/upload-proof.svg" alt="">
                <p>点击或将文件拖拽到这里上传</p>
              </Upload>
          ...
          <div class="next-button">
            <!-- <button class="blue-btn"
                  type='button'
                  @click="nexStepTwo(1)">保存</button> -->
            <button @click="saveInfoFun(2)" type='button' class="white-btn">仅保存不提交</button>
            <button @click="submitInfoFun" type='button' class="blue-btn">确认并提交</button>
          </div>
        </div>
        <div class="step2-box" v-if="steps==2">
          <p class="red-tip">* 若在海关有备案，请使用备案英文信息；若曾申请过国际商标，英文须保持一致。</p>
          <p class="info-title">申请人信息</p>
          <FormItem label="申请人类型" class="inline-box" prop="applyType">
            <RadioGroup v-model="formInfo.applyType">
              <Radio label="1" :disabled="isEditor">公司</Radio>
              <Radio label="0" :disabled="isEditor">个人</Radio>
            </RadioGroup>
          </FormItem>
          <div class="company-info" v-if="formInfo.applyType==1">
            <FormItem label="公司名称（英文）" prop="applyCompanyEn" class="width100">
              <Input v-model="formInfo.applyCompanyEn" :disabled="isEditor" :maxlength="200" style="width:398px;" placeholder="示例：Shenzhen Iipto Technology Co., Ltd."></Input>
              ...
              <p class="gary-tip">推荐使用翻译工具：
                <a href="https://fanyi.sogou.com/" target="_blank">https://fanyi.sogou.com/ </a>或联系我们的
                <a onclick='easemobim.bind({configId: "ecc39d32-7372-4200-af0c-6ef7daf57f5c"})'>客服人员</a>
              </p>
              <p class="gary-tip">最多200字符。</p>
            </FormItem>
            <FormItem label="公司营业执照" prop="businessLicense" class="width100">
              <div class="bread-logo-box" :class="formInfo.businessLicense==''?'':formInfo.businessLicense==undefined?'':'show-change'">
                <Upload :action="workOrderFile" v-if="formInfo.businessLicense==''||formInfo.businessLicense==undefined" class="upload-logo" :on-format-error="formatErrorHandler2" :on-success="(res,file,fileList)=>{successHandler1(res,file,fileList,'businessLicense')}" :on-remove="(file,fileList)=>{removeHandler1 (file,fileList,'businessLicense')}" :format="['jpg','png','bmp']" :show-upload-list="false" :disabled="isEditor">
                  <img src="@/assets/images/infoDetail/upload.svg" alt="">
                </Upload>
                <img :src="formInfo.businessLicense" class="bread-logo" alt="" v-else>
                <Upload class="hover-box" :on-format-error="formatErrorHandler2" :on-success="(res,file,fileList)=>{successHandler1(res,file,fileList,'businessLicense')}" :on-remove="(file,fileList)=>{removeHandler1 (file,fileList,'businessLicense')}" :format="['jpg','png','bmp']" :show-upload-list="false" :disabled="isEditor" :action="workOrderFile">
                  <img src="@/assets/images/infoDetail/editor-bai.png" alt="">更换图片
                </Upload>
              </div>
          ...
          <div class="household-info" v-if="formInfo.applyType==0">
            <Row>
              <Col :span="6">
              <FormItem label="姓氏" :disabled="isEditor" prop="applyLastNameEn">
                <Input v-model="formInfo.applyLastNameEn" placeholder="示例：Zhang" style="width:195px;"></Input>
              </FormItem>
              </Col>
              <Col :span="6">
              <FormItem label="名字" :disabled="isEditor" prop="applyNameEn">
                <Input v-model="formInfo.applyNameEn" placeholder="示例：San" style="width:195px;"></Input>
              </FormItem>
              </Col>
            </Row>
            <div class="ID-file-box">
              <FormItem label="身份证人像面" prop="identityCardFront" class="width156">
                <div class="bread-logo-box" :class="formInfo.identityCardFront==''?'':formInfo.identityCardFront==undefined?'':'show-change'">
                  <Upload :action="workOrderFile" v-if="formInfo.identityCardFront==''||formInfo.identityCardFront==undefined" class="upload-logo" :on-format-error="formatErrorHandler2" :on-success="(res,file,fileList)=>{successHandler1(res,file,fileList,'identityCardFront')}" :on-remove="(file,fileList)=>{removeHandler1 (file,fileList,'identityCardFront')}" :format="['jpg','png','bmp']" :show-upload-list="false" :disabled="isEditor">
                    <img src="@/assets/images/infoDetail/upload.svg" alt="">
                  </Upload>
                  <img :src="formInfo.identityCardFront" class="bread-logo" alt="" v-else>
                  <Upload class="hover-box" :on-format-error="formatErrorHandler2" :on-success="(res,file,fileList)=>{successHandler1(res,file,fileList,'identityCardFront')}" :on-remove="(file,fileList)=>{removeHandler1 (file,fileList,'identityCardFront')}" :format="['jpg','png','bmp']" :show-upload-list="false" :disabled="isEditor" :action="workOrderFile">
                    <img src="@/assets/images/infoDetail/editor-bai.png" alt="">更换图片
                  </Upload>
                </div>
                <p class="gary-tip" style="width:350px;">身份证人像面：请保证身份证姓名、身份证号码清晰可见 身份证国徽面：请保证签发机关、有效期限清晰可见 文件小于20M，支持jpg、png或bmp格式的图片
                </p>
              </FormItem>
              <FormItem label="身份证国徽面" prop="identityCardReverse" class="width156">
                <div class="bread-logo-box" :class="formInfo.identityCardReverse==''?'':formInfo.identityCardReverse==undefined?'':'show-change'">
                  <Upload :action="workOrderFile" v-if="formInfo.identityCardReverse==''||formInfo.identityCardReverse==undefined" class="upload-logo" :on-format-error="formatErrorHandler2" :on-success="(res,file,fileList)=>{successHandler1(res,file,fileList,'identityCardReverse')}" :on-remove="(file,fileList)=>{removeHandler1 (file,fileList,'identityCardReverse')}" :format="['jpg','png','bmp']" :show-upload-list="false" :disabled="isEditor">
                    <img src="@/assets/images/infoDetail/upload.svg" alt="">
                  </Upload>
                  <img :src="formInfo.identityCardReverse" class="bread-logo" alt="" v-else>
                  <Upload class="hover-box" :on-format-error="formatErrorHandler2" :on-success="(res,file,fileList)=>{successHandler1(res,file,fileList,'identityCardReverse')}" :on-remove="(file,fileList)=>{removeHandler1 (file,fileList,'identityCardReverse')}" :format="['jpg','png','bmp']" :show-upload-list="false" :disabled="isEditor" :action="workOrderFile">
                    <img src="@/assets/images/infoDetail/editor-bai.png" alt="">更换图片
                  </Upload>
                </div>
              </FormItem>
          ...
          <FormItem label="国家" :disabled="isEditor" prop="applyCountryEn">
            <Input v-model="formInfo.applyCountryEn" placeholder="示例：Shenzhen Iipto Technology Co., Ltd." disabled></Input>
          </FormItem>
          <FormItem label="省/市" class="red-tip from">
            <Cascader :placeholder="defaultCity" :disabled="isEditor" :data="dataArea" @on-change="getArea"></Cascader>
          </FormItem>
          <Row>
            <Col :span="6">
            <FormItem label="省（英文）" :disabled="isEditor" prop="applyProvinceEn">
              <Input v-model="formInfo.applyProvinceEn" style="width:150px;"></Input>
            </FormItem>
            </Col>
            <Col :span="6">
            <FormItem label="市（英文）" :disabled="isEditor" prop="applyCityEn">
              <Input v-model="formInfo.applyCityEn" style="width:150px;"></Input>
            </FormItem>
            </Col>
          </Row>
          <FormItem label="街道地址（英文）" prop="applyStreetEn" class="width100">
            <Input v-model="formInfo.applyStreetEn" :disabled="isEditor" :maxlength="40" placeholder="示例：Minzhi Street" style="width:398px;"></Input>
            <div class="company-tip-box">
              <img src="@/assets/images/infoDetail/tip.svg" alt="">
              <p>街道地址英文-参考示例：龙华区民治民康路<br> Minzhi Minkang Road, Longhua District</p>
            </div>
            <p class="gary-tip">最多40字符&nbsp;&nbsp; 地理位置查询工具：
              <a href="https://fanyi.sogou.com/" target="_blank">https://fanyi.sogou.com</a>
            </p>
          </FormItem>
          <FormItem label="详细地址（英文）" prop="applyAddresEn" class="width100">
            <Input v-model="formInfo.applyAddresEn" style="width:398px;" :disabled="isEditor" :maxlength="40" placeholder="示例：Room 302, Bldg. 4"></Input>
            <div class="company-tip-box">
              <img src="@/assets/images/infoDetail/tip.svg" alt="">
              <p>详细地址英文-参考示例：3栋402室<br> Room 302, Building 4</p>
            </div>
            <p class="gary-tip">最多40字符&nbsp;&nbsp; 地理位置查询工具：
              <a href="https://fanyi.sogou.com/" target="_blank">https://fanyi.sogou.com</a>
            </p>
          </FormItem>
          <FormItem label="邮政编码" prop="applyPostNum">
            <Input v-model="formInfo.applyPostNum" :disabled="isEditor" :maxlength="6" placeholder="示例：518000"></Input>
            <p class="gary-tip">6位数字</p>
          </FormItem>
          <FormItem label="申请人邮箱" prop="applyEmail">
            <Input v-model="formInfo.applyEmail" :disabled="isEditor" placeholder="示例：123456@hotmail.com"></Input>
          ...
          <div class="legalPersonSignBox">
            <Row>
              ...
              <div class="legalPersonSign">
                <img v-if="formInfo.signature !== '' && formInfo.signature !== undefined" :src="formInfo.signature" alt="">
              </div>
              </Col>
            </Row>
          </div>
          <div class="line"></div>
          <div class="next-button">
            <button @click="steps=1" type='button' class="white-btn">返回上一步</button>
            <button @click="saveInfoFun(2)" v-if="!isEditor" type='button' class="white-btn">仅保存不提交</button>
            <button @click="submitInfoFun" v-if="!isEditor" type='button' class="blue-btn">确认并提交</button>
          </div>
        </div>
        <div class="step3-box" v-if="steps==3">
    ...
    <Modal v-model="proofExampleModal" footer-hide title="示例图" class="proof-example-modal">
      ...
      <p class="modal-tip">
        <img src="@/assets/images/infoDetail/error-icon.png" alt="">不符合原因：1、产品不完整 2、没有logo 3、含有中文 4、图片不清晰
      </p>
    </Modal>
    <Modal v-model="classifyModal" class="classify-modal" footer-hide>
      ...
      <CheckboxGroup @on-change="checkAllGroupChange" v-model="choiceClassify">
        <Checkbox :label="item.categoryNum" v-for="(item,index) in firstClassList" :key="index">{{item.categoryNum
...
export default {
  props: ['formInfo', 'servicesId', 'countryCode'],
  data() {
    return {
      options: {
        disabledDate(date) {
  ...
  methods: {
    ...
    //确认添加二级类目
    sureCheckTwoClassify() {
      this.twoClassifyList.map((item, index) => {
        if (this.selectTwoclassify === item.categoryNum) {
          if (this.twoClassifyNameList.indexOf(this.selectTwoclassify) == -1) {
            this.checkTwoClassifyList.push(item)
            this.twoClassifyNameList.push(this.selectTwoclassify)
          }
        }
      })
    },
    ...
    saveInfoFun() {
      ...
      evidenceUpdateApi(data).then(res => {
        if (res.code == 0) {
    ...
    submitInfoFun() {
      ...
      for (let index = 0; index < this.marketHrefList.length; index++) {
        const element = this.marketHrefList[index];
        if (element.marketHref == '') {
          this.$Message.error({
            content: '销售链接第' + (index + 1) + '项为空，请完善后提交',
            duration: 3,
          })
          return false
        }
        if (element.category2NameEn == '') {
          this.$Message.error({
            content: '销售链接第' + (index + 1) + '项商标小类为空，请选择后提交',
            duration: 3,
          })
          return false
        }
    ...
    submitForm(data) {
      this.$Modal.confirm({
        ...
        content: `<div class="submit-tip-box">
          <p class="submit-tip-title">一旦确认提交后，将会流转到后续注册流程，或产生一定的处理成本。你确定提交吗？</p>
        </div>`,
        okText: '确认',
        cancelText: '取消',
        onOk: () => {
          data.trademarkWorkOrderStatus = 7
          evidenceUpdateApi(data).then(res => {
            if (res.code == 0) {
              this.$emit('backList')
            }
