    ...
    <div class="mainPage"
      ...
      <div class="money">
        <Row class-name="statistical">
          <Col span="3">退款金额: {{totalRefundMoney}}</Col>
          <Col span="3"
               style="float:right;">
          <Button type="primary"
                  style="float:right;"
      ...
      <div class="tableArea">
        <Table border
               ...
               :columns="columns"
               :data="tableData">
          <template slot="refundMethod"
                    slot-scope="{ row }">
            <span>{{row.refundMethod == '1' ? "微信" : row.refundMethod == '2' ? "支付宝" : row.refundMethod == '3' ? "银联" : '' }}</span>
          </template>
        </Table>
...
export default {
  ...
  methods: {
    ...
    getOrderList() {
      ...
      API.financeList(params).then((res) => {
        if (res.code == 0) {
    ...
    statisticsMoney() {
      let params = { ...this.searchForm, ...this.pageInfo };
      API.financeStatistics(params).then((res) => {
        if (res.code === 0) {
          this.totalRefundMoney = res.data.totalRefundMoney
        }
      }).catch(err => console.log(err))
    },
    exportList() {
      ...
      formData.append('refundMethod', this.searchForm.refundMethod == undefined ? '' : this.searchForm.refundMethod)
      ...
      xmlResquest.open('post', this.baseUrl + '/trademark/refund/management/finance/list/export', true)
      // 设置请求token
