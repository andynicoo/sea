<template>
  <div class="invoiceDetail">
    ...
    <div class="orderDetailInfo">
      ...
      <div class="basicInfoBox" v-if="invoiceInfo.companyName !== ''">
        <Row class="rowItem">
          <Col span="4">公司名称：</Col>
          <Col span="4">{{ invoiceInfo.companyName || "暂无" }}</Col>
          <Col span="4">纳税识别号：</Col>
          <Col span="4">{{
            invoiceInfo.taxIdentificationNumber || "暂无"
          }}</Col>
        </Row>
        <Row class="rowItem">
          <Col span="4">公司地址：</Col>
      ...
      <div class="basicInfoBox">
        ...
        <Row class="rowItem">
          <Col span="4">用户级别：</Col>
          <Col span="4">{{
            orderDetailInfo.userType == 0
              ? "普通用户"
              : orderDetailInfo.userType == 1
              ? "成交用户"
              : orderDetailInfo.userType == 2
              ? "渠道用户"
              : orderDetailInfo.userType == 3
              ? "推广用户"
              : "其他"
          }}</Col>
          <Col span="4">电子邮箱：</Col>
        ...
        <Row class="rowItem">
          <Col span="4">订单金额：</Col>
          <Col span="4">{{ orderDetailInfo.orderOldAllMoney }}</Col>
          <Col span="4">优惠金额：</Col>
          <Col span="4">{{ orderDetailInfo.discAmountMoney }}</Col>
        </Row>
        <Row class="rowItem">
          <Col span="4">应付金额：</Col>
          <Col span="4">{{ orderDetailInfo.orderAllMoney }}</Col>
          <Col span="4">订单来源:</Col>
          <Col span="4">{{
            orderDetailInfo.orderBusinessSource == 1
              ? "必胜道"
              : orderDetailInfo.orderBusinessSource == 2
              ? "卖家成长"
              : orderDetailInfo.orderBusinessSource == 3
              ? "跨标云"
              : ""
          }}</Col>
        </Row>
        <Row class="rowItem">
          <Col span="4">实付金额：</Col>
          <Col span="4">{{ orderDetailInfo.amountAllMoney }}</Col>
          <Col span="4">支付渠道:</Col>
          <Col span="4">{{
            orderDetailInfo.paymentChannel == "0"
              ? "未知"
              : orderDetailInfo.paymentChannel == "1"
              ? "微信扫码支付"
              : orderDetailInfo.paymentChannel == "2"
              ? "支付宝PC端支付"
              : orderDetailInfo.paymentChannel == "3"
              ? "银联"
              : orderDetailInfo.paymentChannel == "4"
              ? "微信小程序支付"
              : ""
          }}</Col>
        </Row>
        ...
        <Row class="rowItem">
          ...
          <Col span="4">付款凭证：</Col>
          <Col span="4" v-if="orderDetailInfo.paymentFile">
            <a
