        ...
        <div class="category-box"
          ...
          <div class="form-item">
            ...
            <Input v-model="categoryItem.searchtext"
                   type="textarea"
                   :autosize="{minRows: 4}"
                   @on-change="changeSubcategory($event,categoryIndex)"
                   :disabled="isEditor"
                   style="width:398px;"
                   placeholder="多个小类，每个小类以英文分号隔开。
例子：Acids for etching enamels;Acrylate;Acetanil"></Input>
            <p class="gray-tip">最佳是10-15个商标小类名称，最多20个</p>
          </div>
          <div class="line"></div>
        </div>
         <p class="info-title">使用证据或其他文件</p>
          <Upload :action="workOrderFile"
                  :on-format-error="formatErrorHandler3"
                  :on-success="(res,file,fileList)=>{successHandler2(res,file,fileList,'otherSmallLanguagesFiles')}"
                  :format="['rar','zip','doc','docx','pdf','jpg','xls','xlsx']"
                  :max-size="2048"
                  :show-upload-list="false"
            ...
            <Button>
              <Icon type="ios-cloud-upload"
                    size="22"
                    style="color: #3399ff"></Icon>文件上传</Button>
          </Upload>
          <div v-if="otherFileList.length>0"
             style="margin-top:10px;">
            <p v-for="(item,index) in otherFileList">
              <a  :href="item"
               target="_blank">其它文件{{index+1}}</a>
              <img src="@/assets/images/infoDetail/del.png"
                  style="margin-left:10px;vertical-align: sub;"
                  @click="removeHandler2(index)"
                  alt=""></a>
            </p>
          </div>
          <p style="color:#8d8d8d">支持扩展名：.rar .zip .doc .docx .pdf .jpg .xls .xlsx...</p>
          <p style="color:#8d8d8d">每个文件大小不超过2m</p>
      ...
      <div class="step2-box"
        ...
        <FormItem :label="formInfo.applyType==1?'营业执照上的详细地址':'身份证上的详细地址'"
                  prop="applyAddresEn"
                  class="width100">
...
export default {
  ...
  methods: {
        ...
        this.trademarkCategoryList.map((item, index) => {
          CategoryList.map((v, i) => {
            if (i == index) {
              ...
              let category2List = []
              this.trademarkCategoryList[index].searchtext=''
              if (v.listTrademarkCategory2.length > 0) {
                v.listTrademarkCategory2.map(category => {
                  category2List.push(category.category2NameEn)
                })
                this.trademarkCategoryList[index].searchtext = category2List.join(';')
              }
            }
          })
        })
      }
      if (returnData.otherSmallLanguagesFiles!='') {
        if (returnData.otherSmallLanguagesFiles.indexOf(',')) {
          this.otherFileList=returnData.otherSmallLanguagesFiles.split(',')
        }else{
          this.otherFileList.push(returnData.otherSmallLanguagesFiles)
        }
      }
      this.province = returnData.applyProvinceZh
      this.city = returnData.applyCityZh
      if (returnData.applyProvinceZh!=undefined) {
    ...
    beforeUploadAccept() {
      const check = this.formInfo.otherSmallLanguagesFiles
      let checks = false
      let self = this;
    ...
    formatErrorHandler3() {
      this.$Notice.warning({
        title: "文件格式错误",
        desc: "文件格式不正确, 请上传.rar .zip .doc .docx .pdf .jpg .xls .xlsx格式的文件"
      });
    },
    formatErrorHandler2() {
      this.$Notice.warning({
    ...
    removeHandler1(file, fleList, item) {
      this.formInfo[item] = ''
    },
    successHandler2(res, file, fileList, item) {
      ...
      this.otherFileList.push(res.data.fileUrl)
    },
    removeHandler2(i) {
      this.otherFileList.splice(i, 1)
    },
    changeSubcategory(e,i){
      if (this.trademarkCategoryList[i].category1Num=='') {
        // this.$Message.info('请先选择商标分类')
        this.trademarkCategoryList[i].searchtext=''
        return
      }
    },
    //保存资料
    saveInfoFun() {
      ...
      this.trademarkCategoryList.map((item, index) => {
        item.listTrademarkCategory2 = []
        if (item.searchtext != ''&&item.searchtext!=undefined) {
          let twoCategoryList = item.searchtext.split(';')
          twoCategoryList.map(i => {
            if (i !== '') {
              let obj = {
                category2Name: '',
                category2NameEn: i
              }
              item.listTrademarkCategory2.push(obj)
            }
          })
        }
      })
      if (this.otherFileList.length>0) {
        data.otherSmallLanguagesFiles=this.otherFileList.join(',')
      }
