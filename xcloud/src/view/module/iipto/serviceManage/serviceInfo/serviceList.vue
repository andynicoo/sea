  ...
  <div>
    <div v-if="!showDetail">
      <Form ref="searchForm"
            ...
            inline>
        <FormItem>
          <Input v-model="searchForm.likeName"
                 clearable
                 placeholder="服务名称"
                 style="width:200px;" />
        </FormItem>
      ...
      <Table :data="data"
             :columns="columns"
             border>
        <template slot="functionCode"
                  slot-scope="{row}">
          <span>{{row.functionCode === 1 ? '商标注册' : row.functionCode === 2 ? '答复审查意见' :row.functionCode === 3 ? '商标注册(顾问)' :row.functionCode === 4 ? '补差价':row.functionCode === 5 ? 'R标':row.functionCode === 6 ? '商标转代理':row.functionCode === 7 ? '商标宣誓':row.functionCode === 8 ? '商标转让':row.functionCode === 9 ? '商标复审':row.functionCode === 10 ? '商标复审':row.functionCode === 11 ? '商标修改/增删产品':row.functionCode === 12 ? '使用证据':row.functionCode === 13 ? '商标复活':row.functionCode === 14 ? '商标信息变更':row.functionCode === 15 ? '加速审查':row.functionCode === 16 ? '其他':row.functionCode === 17 ? '专利申请费':row.functionCode === 18 ? '专利加速审查':row.functionCode === 19 ? '专利授权费':row.functionCode === 20 ? '专利加系列' :''}}</span>
        </template>
        <template slot="mainBanner"
                  slot-scope="{row}">
          <img :src="row.productImgUrl"
               class="mainImg">
        </template>
        <template slot="isStart"
                  slot-scope="{row,index}">
          <div>
            <i-switch v-model="row.status==0? true : false"
                      :value="row.status"
    ...
    <goodsDetail v-if="showDetail"
                 :id="id"
                 @refresh="returnList"></goodsDetail>
  </div>
</template>
<script>
    ...
    return {
      ...
      columns: [
        ...
        },
        {
          ...
          key: 'productContent',
          render(h, params) {
            let productContent = JSON.parse(params.row.productContent)
            let text = ''
