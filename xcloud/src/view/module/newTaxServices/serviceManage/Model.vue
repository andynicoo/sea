<template>
  <div class="model">
    <!-- 添加备注 -->
    <el-dialog title="添加备注" :visible.sync="dialog.remarkModel" width="460px">
    ...
    <el-dialog title="订单备注" :visible.sync="dialog.orderRemarkModel" width="495px">
      <div>{{ dialog.orderRemark }}</div>
      <div slot="footer">
        <el-button style="width: 65px" type="primary" @click="dialog.orderRemarkModel = false">关闭</el-button>
      </div>
    </el-dialog>
    <!-- 驳回进度 -->
    <!-- <el-dialog title="驳回进度" :visible.sync="dialog.progressModel" width="530px">
      <el-form ref="ruleForm" label-width="110px" :model="progress">
        ...
        <el-form-item
          label="更改原因："
          prop="remark"
          :rules="[{ required: true, message: '请填写更改原因', trigger: 'blur' }]"
        >
          <el-input
            type="textarea"
            :rows="2"
    ...
    <el-dialog title="修改服务商" :visible.sync="dialog.serviceModel" width="400px" @close="closeModel('supplierForm')">
      <el-form ref="supplierForm" label-width="160px" :rules="supplierForm" :model="supplier">
        ...
        <el-form-item label="选择服务商：" prop="supplierId">
          <el-select v-model="supplier.supplierId" style="width: 168px" clearable filterable>
    ...
    <el-dialog title="驳回详情" :visible.sync="dialog.checkRejectInfoDialog" width="460px" center>
      <el-timeline class="ope-wrapper" v-if="dialog.activities.length > 0">
        <el-timeline-item v-for="(activity, index) in dialog.activities" :key="index" placement="top" hide-timestamp>
          <div class="ope-block">
    ...
    <el-dialog title="修改交付专员" :visible.sync="dialog.taxManagerModel" center width="400px" @close="closeModel('taxManagerForm')">
      <el-form ref="taxManagerForm" :rules="taxManagerForm" :model="taxManager" label-width="140px">
        <el-form-item label="修改交付专员：" prop="disposeId">
          <el-select v-model="taxManager.disposeId" style="width: 168px" filterable>
...
import { orderServiceRemark, deleteRemark, updateSupplier, reject } from '@/api/newApi/taxServices/serviceManage.js';
...
export default {
  ...
  data() {
    return {
      ...
      remarkRuleForm: {
        ...
        annex: [{ required: true, message: '请上传附件', trigger: 'change' }],
      },
      customer: {
        customerServiceId: '',
      },
  ...
  methods: {
    ...
    /** 批量修改交付专员 */
    claimManager() {
      disposeModifyBatch({ disposeId: this.taxManager.disposeId, serviceIdColl: this.dialog.serviceIdColl, serviceId: this.dialog.serviceId }).then((res) => {
        if (res.code === 0) {
          this.$message.success('修改交付专员成功');
    ...
    claimCustomer() {
      this.$refs.customerForm.validate((valid) => {
        if (valid) {
          ...
          disposeCustomerBatch({ customerServiceName, customerServiceId: this.customer.customerServiceId, type: 1, serviceIds: this.dialog.serviceIds }).then((res) => {
            if (res.code === 0) {
    ...
    addServiceRemark() {
      this.$refs.remarkRefForm.validate((valid) => {
        if (valid) {
          orderServiceRemark({
            ...this.remarkObj,
            serviceNo: this.dialog.serviceNo,
