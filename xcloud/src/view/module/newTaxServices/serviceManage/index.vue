<template>
  <div class="app-container">
      ...
      <template #operator>
        <el-dropdown @command="command" style="margin-left: 20px">
          ...
          <el-dropdown-menu slot="dropdown">
            ...
            <el-dropdown-item command="batchReminder" v-if="form.currentNodeCode == currentCodeEnum.NODE2 || form.currentNodeCode == currentCodeEnum.NODE5" v-show="hasAuthority('K1_2')">批量提醒</el-dropdown-item>
            <el-dropdown-item command="batchModifySupplier" v-if="[currentCodeEnum.NODE2, currentCodeEnum.NODE3, currentCodeEnum.NODE7, currentCodeEnum.NODE8].includes(form.currentNodeCode)" v-show="hasAuthority('K1_26')">修改服务商</el-dropdown-item>
            <el-tooltip effect="dark" content="确认转出：指已从服务商/税局转出该公司的申报服务" placement="top">
              <el-dropdown-item command="batchTransfer" v-if="searchStatus === 4" v-show="hasAuthority('K1_24')">一键转出</el-dropdown-item>
            </el-tooltip>
            <el-dropdown-item command="batchDownload" v-if="![undefined, '', currentCodeEnum.NODE2].includes(form.currentNodeCode)" v-show="hasAuthority('K1_32')">下载客户资料</el-dropdown-item>
            <el-dropdown-item command="editLabel" v-show="hasAuthority('K1_34')">设置标签</el-dropdown-item>
    ...
    <BatchDownloadDialog :batchDownloadDialog.sync="batchDownloadDialog" :functionCode="functionCode" contentText="转代理资料" :checkIds="checkIds" />
    ...
    <BatchFinishAgentDialog :batchFinishAgentDialog.sync="batchFinishAgentDialog" :checkIds="checkIds" />
...
import { getPageList, getwayCallback, exportWorkerTax } from '@/api/newApi/taxServices/taxRegister.js';
...
export default {
  ...
  methods: {
    ...
    batchModifySupplier() {
      ...
      if (this.form.currentNodeCode == currentCodeEnum.NODE8 && this.selectList.some((item) => item.declarePeriod !== declarePeriod)) {
    ...
    batchDownload() {
      ...
      if (this.selectList.some((item) => item.productFunctionCode !== this.selectList[0].productFunctionCode)) {
    ...
    batchFinishAgent() {
      ...
      this.batchFinishAgentDialog = true;
    },
    /** 备注弹窗 */
    addRemark(row) {
      this.checkedRow = row;
    ...
    handleClickSetting(row) {
      // 待审核资料 && 德国 跳转新详情
      if (row.currentNodeCode === 'NODE3' && row.countryCode === 'DE' && localStorage.getItem('enterpriseId') === '29' && dayjs(row.flowCreateTime).isAfter(dayjs('2023-07-06 22:30:00'))) {
