...
export default {
  ...
  methods: {
    ...
    /** 批量修改交付专员 */
    claimManager() {
      disposeModifyBatch({ disposeId: this.taxManager.disposeId, serviceIdColl: this.dialog.serviceIdColl, serviceId: this.dialog.serviceId })
        .then((res) => {
          if (res.code === 0) {
    ...
    claimCustomer() {
      this.$refs.customerForm.validate((valid) => {
        if (valid) {
          ...
          disposeCustomerBatch({ customerServiceName, customerServiceId: this.customer.customerServiceId, type: 1, serviceIds: this.dialog.serviceIds })
            .then((res) => {
              if (res.code === 0) {
