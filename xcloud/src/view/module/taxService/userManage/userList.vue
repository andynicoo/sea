<template>
  <div class="userAdmin">
    <Card v-if="!isDetail">
      <Form ref="searchForm"
        ...
        <FormItem label>
          <Select style="width:100px;"
                  ...
                  placeholder="账户状态">
            <Option v-for="item in searchstatecode"
                    :value="item"
                    :key="item">{{item=== 0 ? '禁用' : '启用'}}</Option>
          </Select>
        </FormItem>
        <FormItem label>
          <Select style="width:120px;"
        ...
        <FormItem label>
          <Select style="width:120px;"
            ...
            <Option v-for="item  in salesList"
                    :value="item.userId"
                    :key="item.nickName">{{item.nickName}}</Option>
          </Select>
        </FormItem>
        <FormItem label>
          邀请人：<Input placeholder="请输入邀请人昵称/手机号"
                 style="width:200px;"
      ...
      <Table ref="selection"
        ...
        <template slot="source"
                  slot-scope="{ row ,index }">
          <span>{{row.source == '0' ? 'PC网页端' : row.source == '1' ? '微信小程序' : row.source == '3' ? '微信APP端' : row.source == '5' ? '后台录入' : ''}}</span>
        </template>
        <template slot="operate"
                  slot-scope="{ row ,index }">
          <Button type="primary"
                  ghost
                  @click="toDetail(row.userId)">详情</Button>
          <Button type="primary"
                  ghost
    ...
    <Modal v-model="distribution"
      ...
      <p class="title"
         v-show="modelTitle === '修改跟进人'">当前跟进人：<span class="name">{{currentFollower}}</span></p>
    ...
    <Modal v-model="inviteModal"
      ...
      <Select style="width:100%;"
              ...
              filterable
              placeholder="跟进人">
        <Option v-for="item  in inviteUserList"
                :value="item.userId"
                :key="item.nickName">{{item.nickName}}</Option>
      </Select>
    </Modal>
...
<script>
import { changeUserStatus, getUsersList, updateFollowUsers, changeStatus, roleusers, addFollowRecord, updateInviteUser } from '@/api/taxService/user'
