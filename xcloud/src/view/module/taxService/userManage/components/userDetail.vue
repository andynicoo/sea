    ...
    <Card>
      ...
      <div class="basicInfoBox">
        ...
        <Row class="rowItem">
          ...
          <Col span="4">{{userbaseInfo.userType ==0?'普通用户':userbaseInfo.userType==1?'成交用户':userbaseInfo.userType==2?'渠道用户':userbaseInfo.userType==3?'推广用户':'暂无' }} <span style="color:#16ADE9;cursor:pointer;"
                @click="userLeavelBox=true">修改</span></Col>
          <Col span="4"
...
<script>
import { userOrderList } from '@/api/userclient'
import { getUsersDetail, addFollowRecord, updateUserType } from '@/api/taxService/user'
export default {
  name: 'userDetail',
  props: ['id'],
  data() {
  ...
  methods: {
    ...
    getData() {
      userOrderList(this.searchInfo).then(res => {
        if (res.code === 0) {
          this.data = res.data.records
        }
      })
