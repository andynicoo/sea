    ...
    <Modal title="申请退款"
      ...
      <Form :model="formLeft"
        ...
        <FormItem style="text-align:center;">
          <Button type="primary" :loading="submitApplicationLoading"
    ...
    return {
      ...
      columns: [
        ...
        },
        {
          title: '服务名称',
          render: (h, params) => {
            let beginTime = params.row.suppleDeclStartTime && params.row.suppleDeclStartTime != '' ? params.row.suppleDeclStartTime.slice(0, 7) : '';
            let endTime = params.row.suppleDeclEndTime && params.row.suppleDeclEndTime != '' ? params.row.suppleDeclEndTime.slice(0, 7) : '';
            let biaoShi = params.row.functionCode && params.row.functionCode == 17 ? '补申报区间：'+ beginTime + '至' + endTime: ''
            return h('div', [
        ...
        },
        {
          title: '服务公司',
          render: (h, params) => {
            return h('div',params.row.functionCode == 17? params.row.suppleCompanyName:params.row.companyName)
          },
        },
        {
          title: '服务状态',
