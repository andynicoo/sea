    ...
    <div class="orderListWrapper">
        <div class="mainPage"  v-if="!isDetailShow">
            <Form ref="searchForm" :model="searchForm" inline>
                ...
                <!-- <FormItem>
                    <Select clearable v-model="searchForm.status" style="width: 100px;" placeholder="退款状态">
            ...
            <div class="tableArea">
                <Tabs :value="searchForm.status" :animated="false" @on-click="tabsChange">
                    ...
                    <TabPane label="待退款" name="1"></TabPane>
                    <TabPane label="已退款" name="2"></TabPane>
                    <TabPane label="审核不通过" name="3"></TabPane>
                </Tabs>
                <Table
                border 
                ...
                <template slot="operate" slot-scope="{ row }">
                    <Button ghost type="info" v-show="hasAuthority('order_refund01')" @click="toDetail(row)">查看</Button>
                    <Button ghost type="info" v-show="hasAuthority('order_refund02')" v-if="row.status === 0"  @click="auditFun(row.id)">审核</Button>
                    <Button ghost type="info" v-show="hasAuthority('order_refund03')" v-if="row.status === 1" @click="refundFun(row)">确认退款</Button>
                </template>
                </Table>
                <Page
        ...
        <refundDetail v-if="isDetailShow" :rowInfo='rowInfo' @back="backFun" @refresh="refreshList"></refundDetail>
        <auditMedal v-if="isAuditMedalShow" @changeState="auditMedalFun" :refundId="refundId" @refresh="refreshList"></auditMedal>
        <refundMedal v-if="isRefundMedalShow" @changeState="auditMedalFun" :refundInfo="refundInfo" @refresh="refreshList"></refundMedal>
    </div>
</template>
<script>
