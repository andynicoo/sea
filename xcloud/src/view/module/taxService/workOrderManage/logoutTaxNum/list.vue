  ...
  <div class="container">
    <div class="mainPage" v-if="!isDetailShow">
      <Form ref="searchForm" class="formContainer" :model="searchForm" inline>
        ...
        <FormItem>
          <Select clearable v-model="searchForm.countryCode" @on-change="countryChange" placeholder="注销国家">
      ...
      <div class="tab">
        <Button type="primary" style="margin-right:10px;" v-show="hasAuthority('logoutTaxNum_batchClaimTaxManager')" @click="openClaimTaxManagerModal()">修改税务经理</Button>
      </div>
      <div class="tableArea">
        <Tabs :value="searchForm.status" :animated="false" @on-click="tabsChange">
          <TabPane label="全部" name="-1"></TabPane>
          <TabPane label="待上传资料" name="51"></TabPane>
          <TabPane label="待审核" name="52"></TabPane>
          <TabPane label="注销中" name="53"></TabPane>
          <TabPane label="已完成" name="54"></TabPane>
          <TabPane label="资料驳回" name="55"></TabPane>
          <TabPane label="注销失败" name="56"></TabPane>
        ...
        <Table border :loading="tableLoading" :columns="columns" :data="tableData" @on-selection-change="selectChange">
          <template slot="operate" slot-scope="{ row }">
            ...
            <Button type="primary" ghost size="small"  @click="toRemark(row)" v-show="hasAuthority('logoutTaxNum_remark')">备注</Button>
          </template>
          ...
          </template><template slot="supplierName" slot-scope="{ row }">
            <div class="supplierName">
              <span v-show="hasAuthority('logoutTaxNum_lookSupplier')">{{row.supplierName}}</span>
              <!-- <Icon v-if="[51,52].includes(row.progressBar)" class="icon" type="md-create" v-show="hasAuthority('logoutTaxNum_editSupplier')" @click="openEditSupplierModal(row)"></Icon> -->
            </div>
          </template>
          <template slot="disposeName" slot-scope="{ row }">
            <div class="disposeName">
              <span v-if="row.disposeId">{{row.disposeName}}</span>
              <!-- <span v-if="!row.disposeId" class="button" v-show="hasAuthority('logoutTaxNum_claimTaxManager')" @click="openClaimTaxManagerModal(row)">认领</span> -->
            </div>
          </template>
    ...
    <Modal v-model="remarkListModal" title="备注信息">
      <div v-if="remarkList.length>0" :class="remarkList.length>2?'overflow':''" style="padding-right: 15px;">
        <div class="remark-list" v-for="(item,index) in remarkList" :key="index">
          ...
          <span class="delet" @click="deletRemarkHandel(item.id,index)" v-show="hasAuthority('logoutTaxNum_deletRemark')">X</span>
        </div>
      </div>
    ...
    <Modal v-model="refuseReasonModal" title="驳回详情">
      <div v-if="refuseReasonList.length>0" :class="refuseReasonList.length>2?'overflow':''" style="padding-right: 15px;">
    ...
    <!-- 详情 -->
    <Details v-if="isDetailShow" :serviceId="serviceId" @back="isDetailShow = false" @refresh="getList"></Details>
  </div>
</template>
<script>
...
export default {
      ...
      columns_showSupplierName: [
        ...
        },
        {
          ...
          render(h, params) {
            ...
            serviceName = code == 1 ? '税务注册' : code == 2 ? '税务申报' : code == 3 ? '税务补缴' : code == 4 ? '转代理' : code == 5 ? '注册+申报' : code == 6 ? '注销税号' : ''
  ...
  computed: {
    columns(){
      if(this.hasAuthority('logoutTaxNum_lookSupplier')){
  ...
  methods: {
    ...
    getList() {
      ...
      API.canceltaxList(params).then((res) => {
    ...
    openEditSupplierModal(row) {
      ...
      let params = {};
      if (row) {
        if (![51, 52].includes(row.progressBar)) {
        ...
        if (this.selectList.some(el => ![51, 52].includes(el.progressBar))) {
          this.$Message.warning('仅进度为待上传注册资料和待审核的工单可以修改服务商！')
          return false;
        }
