        ...
        <div class="itemList">
          <div class="item">
            ...
            <div class="item-content">
              {{saleData.toperationClewCount || 0}}
          ...
          <div class="item">
            ...
            <div class="item-content">
              {{saleData.taxVatServiceTargetCount || 0}}
              <span class="item-unit">
                个
              </span>
            </div>
        ...
        <div class="departmentContent">
          ...
          <div class="content">
            <!-- <span style="margin-right:10px">
              <Select v-model="saleUserDataObj.countryCode" style="width:200px" placeholder="全部国家">
                <Option 
            ...
            <DatePicker type="daterange"
                        :value="[this.saleUserDataObj.startTime, this.saleUserDataObj.endTime]"
                        @on-change="saleUserDataChange"
                        placeholder="Select date"
                        style="width: 200px"></DatePicker>
            <Button type="primary"
                    @click="getsaleUserData"
                    style="margin-left:10px;">查询</Button>
            <Table height="420"
                   style="margin-top:20px;"
                   :columns="operationUserColums"
                   :data="saleUserData"></Table>
            <Table height="420"
                   style="margin-top:20px;"
                   :columns="operationUserColums2"
                   :data="saleUserData"></Table>
            <div style="margin-top:20px;">
              <Select clearable
                      v-model="saleViewObj.departmentId"
              ...
              <Select clearable
                      v-model="saleViewObj.departmentUserId"
                      style="width: 100px;margin-right:10px;"
                      placeholder="选择人员">
...
export default {
  ...
  data() {
    return {
      ...
      operationUserColums2: [
        ...
        {
          title: '线索转化注册数',
          key: 'payUserClewAmount',
          sortable: true
        },
        {
          title: '线索成交用户数',
          key: 'registerUserClewCount',
          sortable: true
        },
        {
          title: '税号数',
  ...
  methods: {
    ...
    // },
    getsaleData() {
      if (this.saleDataObj.year === '' && this.saleDataObj.month === '') {
        this.$Message.error('请选择时间')
        return
      }
      saleApi(this.saleDataObj).then((res) => {
        if (res.code === 0) {
    ...
    draw() {
      ...
      let xAxisList = []
      if (that.saleViewObj.month === undefined) {
        for (let index = 1; index < 13; index++) {
          let item = index + '月'
          xAxisList.push(item)
        }
      ...
      if (this.saleviewData.length > 0) {
        for (let i = 0; i < this.saleviewData.length; i++) {
          departmentNameList.push(this.saleviewData[i].departmentName)
          seriesList[i] = {
            name: `${this.saleviewData[i].departmentName}`,
            type: 'line',
            smooth: true,
            lineStyle: {
              color: colorList[i],
            ...
            data: this.saleviewData[i].totalList,
          }
        }
      } else {
        seriesList = [{
      ...
      let option = {
        ...
        tooltip: {
          ...
          formatter: function (obj) {
            let data = {}
            for (let index = 0; index < that.saleviewData.length; index++) {
              const element = that.saleviewData[index];
              if (element.departmentName === obj.seriesName) {
                data = JSON.parse(JSON.stringify(element))
              }
            }
            let departmentUserName = ''
            if (that.saleViewObj.departmentUserId) {
            ...
            let text =
              ` <div style="padding:0px 10px;">
                    <div>${obj.seriesName}${departmentUserName}${obj.name}数据</div>
    ...
    getCurrentMonthFirst() {
      ...
      if (day < 10) day = '0' + day
      this.saleUserDataObj.startTime = date.getFullYear() + '-' + month + '-' + day
    ...
    getCurrentMonthLast() {
      ...
      if (month < 10) month = '0' + month
      if (day < 10) day = '0' + day
      this.saleUserDataObj.endTime = date.getFullYear() + '-' + month + '-' + day
      return date.getFullYear() + '-' + month + '-' + day
    },
    operatDateChange(val) {
      this.saleDataObj.year = val
    },
    operatYearDateChange(val) {
      this.saleViewObj.year = val
    },
    saleUserDataChange(val) {
      this.saleUserDataObj.startTime = val[0]
      this.saleUserDataObj.endTime = val[1]
    },
    compare(property) {
      return function (a, b) {
    ...
    getsaleUserData() {
      saleUserApi(this.saleUserDataObj).then(res => {
        if (res.code === 0) {
          if (JSON.stringify(res.data) !== '{}') {
            this.saleUserData = res.data
          } else {
    ...
    getOperatViewData() {
      saleViewApi(this.saleViewObj).then((res) => {
        if (res.code === 0) {
          if (JSON.stringify(res.data) !== '{}') {
    ...
    departmentChange(val) {
      this.saleViewObj.departmentId = val
      this.departmentUserList(val)
    },
  },
  created() {
    this.saleDataObj.month = this.currentMonth
    this.saleDataObj.year = this.currentYear
    this.saleViewObj.year = this.currentYear
    this.saleViewObj.month = this.currentMonth
