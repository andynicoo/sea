      ...
      <Card shadow>
        <tree-table style="max-height:700px;overflow: auto"
          ...
          <template slot="status" slot-scope="scope">
            ...
            <span>
              <Icon v-if="scope.row.icon.indexOf('side') < 0" :type="scope.row.icon" size="16"/>
              <i v-else :class="['icon',scope.row.icon]" />
            </span>
          </template>
        </tree-table>
      </Card>
        ...
        <Form ref="menuForm" :model="formItem" :rules="formItemRules" :label-width="80">
          ...
          <FormItem label="图标">
            ...
            <Poptip width="600" slot="append" placement="bottom">
              ...
              <div slot="content">
                <ul class="icons">
                  <li class="icons-item" :title="item" @click="onIconClick(item)" v-for="(item,index) in selectIcons" v-bind:key="`${item}${index}`">
                    <Icon :type="item" size="28"/>
                    <p>{{item}}</p>
                  </li>
                  <li class="icons-item " :title="item" @click="onIconClick(item)" v-for="(item,index) in selectIcons2" v-bind:key="`${item}${index}`">
                    <div class="icons-item-side">
                      <i :class="['icon',item]" class="side-item-icon"/>
                    </div>
                    <p>{{item}}</p>
                  </li>
                </ul>
