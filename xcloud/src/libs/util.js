...
export const getBreadCrumbList = (route, homeRoute) => {
  ...
  let res = routeMetched
    .filter((item) => {
      return item.meta === void 0 || !item.meta.hide;
    })
    .map((item) => {
      let meta = { ...item.meta };
    ...
    if (item.parentId === 0 || item.parentId === '0') {
      ...
      if (!hasChild(item)) {
        ...
        if (item.target === '_blank') {
          ...
          if (item.scheme === '/') {
            if (qianKunConfigApps.map((v) => v.activeRule).some((v) => ('/' + item.path).indexOf(v) !== -1)) {
              //微服务组件
              router.component = (resolve) => {
                require([`@/view/module/qiankun/index.vue`], resolve);
              };
            } else {
              // 内部组件
              router.component = (resolve) => {
