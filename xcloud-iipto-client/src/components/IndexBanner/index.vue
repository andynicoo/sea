<template>
  <div v-if="bannerList.length > 0">
    <a-carousel :after-change="onChange" v-if="bannerList.length > 1" :autoplay="flagCarousel">
...
<script>
import { getAdByCode, getAdDetails } from '@/api/login/login';
export default {
  name: 'IndexBanner',
  data() {
    return {
  ...
  mounted() {
    this.getAdList();
    getAdDetails().then((res) => {
      let bannerDetails = res.data.filter((item) => this.adKey == item.adKey);
      if (bannerDetails.length) {
        this.flagCarousel = bannerDetails[0].flagCarousel;
      }
    });
  },
  methods: {
