<template>
  <div class="mainContent">
    <div class="my-card" :style="contentStyleObj">
      ...
      <!-- <div class="topImg">
        <a href="inviteSubject">
          <img src="@/assets/images/activity/invite.png"
               alt="">
        </a>
      </div> -->
      <Table style="min-height:184px;margin-top:10px" no-data-text="暂无邀请用户" border :columns="columns" :data="inviteList">
        <template slot="operate" slot-scope="{ row }">
          <span @click="toRegister(row)" class="operate">去处理</span>
        </template>
      </Table>
      <div class="page-box">
        <Page :current="currentPage" :total="listTotal" @on-change="registerChangePageHandler" @on-page-size-change="registerPageSizeChangeHandler" :page-size="perPageCount" show-total />
      </div>
    </div>
    <Modal v-model="inviteBox" :mask-closable="false" footer-hide :width="640" class-name="vertical-center-modal">
      ...
      <div class="invit-content">
        ...
        <p class="link-btn cobyOrderSn" data-clipboard-action="copy" :data-clipboard-text="shareLink" @click="copyLink">复制链接</p>
        <!-- <div class="active-rule">
                    <p class="rule-title">活动规则</p>
                    <p class="rule-content">一：邀请三个用户成功注册跨标云账号，即可免费注册一个英国税号；<br> 二：优惠券仅限本账号使用，不能转让；<br> 三：优惠券有效期限：30天；<br> 四：活动有效期：2020.3.19-2020.4.19；</p>
                </div> -->
      </div>
    </Modal>
  </div>
...
export default {
  ...
  methods: {
    ...
    getUserService () {
      ...
      this.shareLink =
        this.homePageUrl + '?invitedCode=' + userInfo.invitedCode
    },
    copyLink () {
      let _this = this
