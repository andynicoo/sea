<template>
  <div class="container" id="page" ref='page'>
    ...
    <Modal v-model="prizeModal" width="370" class="win-modal" :closable="false">
      ...
      <template #footer>
        <div class="know-btn" @click="prizeModal=false">{{myPrizeLevel===3?'我知道了':'我知道了'}}</div>
      </template>
    </Modal>
    <Modal v-model="winningModal" footer-hide class-name="modal-winner" :transfer="false" width="360">
      <div class="modal-winner-content">
        <img class="prize-img" v-if="+drawLevel===1" src="./images/bg-2.png" alt="">
        <img class="prize-img" v-if="+drawLevel===3" src="./images/bg-1.png" alt="">
      </div>
      <div class="modal-winner-footer">
        <Button class="btn btn-ok" v-if="+drawLevel!==3" @click="winningModal=false;prizeModal=true">查看奖品</Button>
        <Button class="btn btn-ok" v-if="canPrize" @click="handlePrizeMore">再刮一次</Button>
...
import {getTimesAPI, getDrawResultAPI, getDrawRecordAPI} from '@/api/618activity/index'
import { activeTempTimes } from '@/config/index.js'
...
export default {
  ...
  data () {
    return {
      ...
      rules: [
        ...
        '参与用户：跨标云平台所有注册用户',
        '所有在2022年9月6日9:00-2022年9月8日24:00下单用户均可参与抽1000元美标立减优惠券；优惠券有效期至2022年9月30日24:00，过期失效，不退不换',
        '由于商品属性，所有秒杀商品不退不换',
  ...
  computed: {
    drawText () {
      let text = ''
      let startTime = new Date(activeTempTimes.start)
      let endTime = new Date(activeTempTimes.end)
    ...
    canPrize () {
      return (this.showJoin && this.prizeCount > 0)
    }
  },
  mounted () {
