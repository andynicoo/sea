...
const ExpertDetailPage: NextPage<ExpertDetailPageProps> = (props) => {
    ...
    let personalBg = '/asset/image/user_banner.png'
    if (selfInfo && selfInfo.userId == userInfo.userId) {
        personalBg = userInfo.personalCenterBackground
    }
    const bannerCls = css`
        width: 100%;
        height: 183px;
        background-image: url(${personalBg});
    ...
    return (
        <ExpertDetailContext.Provider value={ctxValue}>
            <MinHead />
            <SeoHead
                title={`${userInfo.nick}的个人主页_${userInfo.nick}简介、微信公众号_跨境知道`}
                keyword={`${userInfo.nick},${userInfo.nick}的个人主页,${userInfo.nick}简介,${userInfo.nick}公众号`}
                description={`${userInfo.nick}的个人主页，${userInfo.nick}简介 ：${userInfo.individualResume}`}
            />

            {/* 顶部banner */}
            <div className={bannerCls} style={{ height: "183px" }} />
