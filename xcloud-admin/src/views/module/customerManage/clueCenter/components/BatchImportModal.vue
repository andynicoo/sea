    ...
    <div class="wrap">
      <a-tabs v-model:activeKey="activeKey" @change="changeTags">
        ...
        <a-tab-pane :key="2" tab="导入日志">
          <DataTable ref="dataTable" :columns="columns" :methodName="selectBatchThreadList" :mainTable="false">
...
import {
  verificationTemplateDownload,
  addDataBatchTask,
  selectBatchThreadList,
  downloadRecord,
} from '@/api/customerManage/clueCenter';
...
//单个下载
const downloadRecordFun = (record: any) => {
  downloadRecord({
    path: record.newAddress,
  }).then((res: any) => {
    downloadFile(res, '文件.xlsx');
  });
};
