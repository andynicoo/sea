<template>
  <div>
    ...
    <div class="zero" v-if="infoNews.length === 0">
      当期 0 申报
    </div>
    <Table v-else :columns="columnsEsBattProduct" :data="infoNews" border max-height="500" no-data-text="暂无数据" ></Table>
    
    <div class="title" >当年申报数据汇总</div>
    <Table :columns="columnsEsBattProduct" max-height="500" :data="summaryShowData" class="summary" border no-data-text="暂无数据">    </Table>
  </div>
</template>
<script>
import { columnsEsBattProduct } from '../configES';
...
export default {
  ...
  methods: {
    init(){
      this.summaryShowData = [].concat(this.summary.infoNews) 
...
<style lang="less" scoped>
  .title {
    ...
    padding: 16px 0;
    font-size: 18px;
    background: rgba(@table-bg-color, 0.2);
