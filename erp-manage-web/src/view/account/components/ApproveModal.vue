...
import { moneyAgree, supportChannel } from '@/api/account'
...
export default {
      ...
      if (data.isOnline) {
        arr = pullicArr.concat([
          ...
          },
          {
            ...
            label: '代付渠道',
            itemType: 'select',
            itemData: this.payChannelList.map((v) => {
              return {
                label: v.payChannelDes,
                value: v.payChannel
              }
            }),
            required: true,
  ...
  methods: {
    ...
    async setFormModel() {
      try {
        this.payChannelList = (await supportChannel({})).data || []
      } catch (error) {
        console.error(error)
      }
      const val = this.data
      ...
      if (this.actionType === 'edit') {
        ...
        // 设置默认
        for (const li of this.payChannelList) {
          if (li.channelDefault) {
            this.formModel.payChannel = li.payChannel // 'xsPay'
          }
        }
