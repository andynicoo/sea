  ...
  <div ref="container" class="wrapper">
    ...
    >
      ...
      <template #action="record">
        <span
          v-permission:case_manager="['edit']"
          class="action-button"
    ...
    <ClassMavager v-model="visible" :main-type="mainType" @ok="getClassifySelect" />
  </div>
</template>
...
import { newsOrCashList, newsOrCashDelete, classifySelect } from '@/api/website'
  ...
  methods: {
    ...
    handleDelete(record) {
      this.$confirm({
        ...
        content: '确定删除吗?',
        onOk: () => {
          this.caseDelete([record.id])
        }
      })
    },
    async caseDelete(ids) {
      const params = {
        mainType,
        ids
      }
