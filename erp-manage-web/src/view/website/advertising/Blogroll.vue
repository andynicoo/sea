<template>
  <div class="wrapper">
    <div ref="filters" class="filters">
      ...
      >
        <template #button>
          ...
          <a-button v-permission:advertising="['add']" type="primary" @click="handleAction('add')">新增</a-button>
        </template>
      </TableFilter>
    </div>
    <a-table
    ...
    <BlogrollModal v-model="visible" :data="currentObj" :action-type="actionType" />
  </div>
</template>

<script>
import TableFilter from '@/components/tableFilter'
import BlogrollModal from './components/BlogrollModal'
import { debounce, setTableHeight } from '@/utils'
import { advertisingList, advertisingDelete } from '@/api/website'
  ...
  methods: {
    ...
    handleDelete(record) {
      this.$confirm({
        ...
        onOk: async() => {
          try {
            await advertisingDelete({ type, ids: [record.id] })
