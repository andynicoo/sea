<template>
  <div class="content">
    <div class="commontitle">领券中心</div>
    <div class="my-card">
      ...
      <div v-if="orderList.length > 0">
        <div class="myVATFilter itemList">
          <ul>
            <li
              class="item"
              :class="item.hasReceived ? 'has-received' : ''"
              v-for="(item, index) in orderList"
              :key="index"
              @click="userSeceive(item)"
            >
...
import { selectCoupon, reactiveCoupon } from '@/api/activity/index';
  ...
  methods: {
    ...
    getCouponList() {
      coupponList({
        ...
        let couponList = res.data.filter((coupon) => coupon.status === 'UNUSED');
        if (res.code === 0) {
          this.orderList.forEach((item) => {
            if (couponList.find((coupon) => coupon.marketingCouponActivityId === item.marketingCouponActivityId)) {
...
.my-card {
  ...
  .itemList {
    ...
    .item {
      ...
      cursor: pointer;
      &.has-received {
        background: url('../../../assets/images/coupon/use_coupon_bg.svg') no-repeat center;
        background-size: 100%;
      }
    }
  }
