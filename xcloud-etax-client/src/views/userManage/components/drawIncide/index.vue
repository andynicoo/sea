        ...
        <div v-else>
          ...
          <FormItem label="开票类型">
            <RadioGroup v-model="contractType">
              <Radio :disabled="isWAIT_PAY" label="2">增值税普通发票（个人） </Radio>
              <Radio :disabled="isWAIT_PAY" label="3">增值税普通发票（公司）</Radio>
              <Radio :disabled="isWAIT_PAY" label="4">增值税专用发票</Radio>
            </RadioGroup>
          </FormItem>
        </div>
        <!-- 增值发票个人 || 后补发票时显示 -->
          ...
          <Row class="formBox">
            <FormItem prop="idOrTaxNumber" label="身份证号码">
              <Input
                :disabled="isWAIT_PAY"
                style="width: 266px"
                v-model="invoiceForm.idOrTaxNumber"
        ...
        <template v-if="!isBefore">
          <div class="payType clearfix">
            <PayModeCom
              ...
              payMethodParams="CHN_PAY"
            />
          </div>
          <div class="tax">
            税额:
            <span v-if="isWAIT_PAY">¥ {{ invoiceForm.makeInvoiceMoney }}</span>
            <span v-else
              >¥
              <TaxRate
                :contractTypeEM="getInvoice.invoiceType"
                :orderPaidMoney="orderInfo.orderPaidMoney || orderInfo.orderPaymentMoney"
...
<script>
import { getInvoiceDetails, invoice, invoiceFillMaterial } from '@/api/user/myOrder';
...
export default {
  ...
  created() {
    ...
    console.log(this.orderInfo.invoiceVO.status, 'is.orderInfo.invoiceVO.stat');
    if (this.orderInfo.invoiceVO.status === 'WAIT_PAY') {
      this.isWAIT_PAY = true;
    }
  },
  methods: {
    async submit() {
    ...
    pay() {
      ...
      if (Number(this.tax) == 0) {
        ...
        userPay('PC', clientOrderProductVOList, { amount: this.tax, ...this.payMode, orderNo }, (payResponse) => {
          this.dialog = false;
    ...
    async init() {
      ...
      if ((this.isBefore && orderId) || (JSON.stringify(invoiceVO) != '{}' && invoiceVO.status == 'WAIT_PAY')) {
        ...
        if (this.isWAIT_PAY) {
          this.tax = this.invoiceForm.makeInvoiceMoney;
        }
        if (data.invoiceType === 'SPECIAL') {
          this.contractType = '4';
        } else if (data.invoiceType === 'GENERAL' && data.subjectType === 'ENTERPRISE') {
          this.contractType = '3';
        }
      }
    },
