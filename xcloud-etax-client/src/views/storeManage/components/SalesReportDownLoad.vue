<template>
  <div>
    <Modal v-model="showModal" title="查看报告" :mask-closable="false" :footer-hide="true" width="1000">
      <div class="wrap">
        ...
        <Table border :columns="columns" :data="listData">
          ...
          <template slot="operate" slot-scope="{ row }">
            <Button size="small" v-if="row.status === 1" @click="dowmLoadReport(row)" type="primary">下载报告</Button>
            <Button size="small" v-else @click="createAgain(row)" type="primary">重新生成</Button>
          </template>
        </Table>
      </div>
    </Modal>
...
<script>
import { reCreateReport } from '@/api/store';
export default {
  data() {
    return {
      showModal: false,
