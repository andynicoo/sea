<template>
  <div>
    <div class="head">
      <div class="epr-title" v-if="status == nodeEnum.WAIT_UPLOAD">
        <!-- <span class="left">待上传注册资料</span>
      ...
      <div v-else>
        ...
        <div class="tabTitle">
          <span
            ...
            v-if="
              status == nodeEnum.REGISTER_COMPLETE ||
              status == nodeEnum.REGISTERING ||
              status == 'NODE0365' ||
              status == 'NODE0366'
            "
            @click="pageType = 1"
    ...
    <div v-if="pageType == 2">
      <DynamicForm
        ...
        :tabsCurrent="pageType"
      />
    </div>
    <!-- 英代欧代 -->
    <div v-if="pageType == 1 && orderServices.productFunctionCode == 16">
      <div class="finish">
        <div class="top">
          <p>
            <img src="@/assets/images/compliance/lanse.svg" />
            温馨提示：{{ serviceName }}已经注册成功，您可以：
          </p>
          <p class="right">1、将欧代/英代的公司名及公司地址填写在亚马逊后台</p>
          <p class="right">2、将欧代/英代的公司名、地址、联系方式，印刷在有CE认证的产品外包装上</p>
        </div>
        <div class="bottom">
          <div class="one">
            <div>
            ...
            <div v-if="orderServices.companyNameEn">
              <p class="left">公司英文名称：</p>
              <p class="right">{{ orderServices.companyNameEn || '暂无' }}</p>
            </div>
            <div>
              <p class="left">国家：</p>
          ...
          <div class="two">
            <div>
              <p>
                ...
                <a
                  class="chakan"
                  :href="authorizedAgentWorkOrder.tagFile"
                  target="_blank"
                  v-if="authorizedAgentWorkOrder.tagFile"
                >
              ...
              <p style="margin-top: 16px">
                ...
                <span class="right">
                  {{
                    (authorizedAgentWorkOrder.agreementBeginTime &&
                      authorizedAgentWorkOrder.agreementBeginTime.slice(0, 10)) ||
                    '暂无'
                  }}
                </span>
              </p>
            ...
            <div>
              <p v-if="authorizedAgentWorkOrder.certificat">
                <span class="left">{{ countryNameZh == '欧盟' ? '欧代证书：' : '英代证书：' }}</span>
                <a
                  v-for="(item, index) in $util.convertImage(authorizedAgentWorkOrder.certificat)"
                  :key="index"
                  class="chakan"
                  :href="item"
                  target="_blank"
              ...
              <p style="margin-top: 16px">
                ...
                <span class="right">
                  {{
                    (authorizedAgentWorkOrder.agreementEndTime &&
                      authorizedAgentWorkOrder.agreementEndTime.slice(0, 10)) ||
                    '暂无'
                  }}
                </span>
              </p>
            ...
            <div>
              ...
              <p v-if="authorizedAgentWorkOrder.agreement" style="margin-top: 16px">
                <span class="left">{{ countryNameZh == '欧盟' ? '欧代协议：' : '英代协议：' }}</span>
                <a
                  v-for="(item, index) in $util.convertImage(authorizedAgentWorkOrder.agreement)"
                  :key="index"
                  class="chakan"
                  :href="item"
                  target="_blank"
    ...
    <div
      v-if="
        pageType == 1 &&
        [31, 3101, 3102, 3103, 3104, 3105, 3106, 3107, 3108].includes(orderServices.productFunctionCode)
      "
    >
      <div class="finish">
        <div class="bottom eprBottom">
          <div style="margin-top: 0" class="sub-title">基础信息</div>
          <div class="one">
            <div>
              <p class="left">注册主体名称：</p>
              <p class="right">{{ orderServices.companyNameZh || authorizedAgentWorkOrder.naturalPerson || '暂无' }}</p>
          ...
          <div class="sub-title">授权信息</div>
          <div class="one" v-if="info.authorizeColumns.length">
            <div v-for="item in info.authorizeColumns" :key="item.columnId">
              <p class="left">{{ item.keyLabel }}：</p>
              <p class="right">
                <span v-if="item.reView">
                  <a class="chakan" :href="item.reView" target="_blank">
                    <img src="@/assets/images/compliance/chakan.svg" />
                    查看
                  </a>
                </span>
          ...
          <div class="sub-title">证书信息</div>
          <div class="one">
            <div>
              <p class="left">证书：</p>
              <div class="right">
                <p v-if="eprWorkOrderInfo.registrationReceipt">
                  <span
                    v-for="(item, index) in $util.convertImage(eprWorkOrderInfo.registrationReceipt)"
                    :key="index"
                    class="chakan"
                    @click="seeReceipt(item)"
                  >
                    <img src="@/assets/images/compliance/chakan.svg" />
                    查看
                  </span>
            ...
            <div>
              <p class="left">UIN号：</p>
              <p class="right" v-if="uinStr">
                {{ uinStr }}
                <a
                  style="margin-left: 10px"
                  id="uinStr"
                  data-clipboard-action="copy"
                  :data-clipboard-text="uinStr"
                  @click="copyUin"
                  class="chakan"
                  >复制</a
                >
              </p>
              <p class="right" v-else>暂无</p>
            </div>
            <div>
          ...
          <div class="one">
            <div>
              <p class="left">账单：</p>
              <div class="right">
                <p v-if="eprWorkOrderInfo.billFiles">
                  <a
                    v-for="(item, index) in eprWorkOrderInfo.billFiles"
                    :key="index"
                    class="chakan"
                    :href="item"
                    target="_blank"
            ...
            <div>
              <p class="left">缴费凭证：</p>
              <p class="right">
                <template v-if="eprWorkOrderInfo.voucherFiles.length">
                  <a
                    v-for="(item, index) in eprWorkOrderInfo.voucherFiles"
                    :key="index"
                    class="chakan"
                    :href="item"
                    target="_blank"
    ...
    <div
      v-if="
        pageType == 3 &&
        [16, 31, 3101, 3102, 3103, 3104, 3105, 3106, 3107, 3108].includes(orderServices.productFunctionCode)
      "
    >
      <!-- 待授权 -->
      <EprAuthorizer
    ...
    <Modal v-model="refuseReasonModal">
      ...
      <div class="tipsModal-contnet">
        <p class="tip-title">
          您的{{ status === nodeEnum.AUTHORIZE_WAIT_CONFIRM ? '授权' : '资料' }}审核未通过，请按照以下指引修改...
        </p>
...
<script>
import { getDetail, getNodeField, getHideInfo } from '@/api/taxRegister.js';
...
import { speedList } from './dateList';
...
export default {
  ...
  computed: {
    ...
    uinStr() {
      if (!this.hideInfo) {
        return this.eprWorkOrderInfo.uinStr;
      }
      if (!this.eprWorkOrderInfo.uinStr) {
        return '********';
      }
      const len = this.eprWorkOrderInfo.uinStr.length;
      const lenRep = len - 4 > 0 ? len - 4 : 0;
      const respStr = '*'.repeat(lenRep);
      return this.eprWorkOrderInfo.uinStr.substring(0, 4) + respStr;
    },
    statusList() {
      let nodeList = this.forwardNodes.map((item) => item.nodeCode);
      return speedList.filter((item) => nodeList.includes(item.nodeCode));
    },
    orderStatus() {
  ...
  methods: {
    ...
    seeReceipt(item) {
      /** 下载证书 */
      // this.$util.downFile(item);
      if (this.hideInfo) {
        this.$Modal.info({
          title: '提示',
          content: `您购买的合规服务“${this.serviceName}”由于23年官费上涨，您未进行差额补缴，暂无法下载注册证书+查看UIN号码，请联系客户经理进行补缴，下载查询权限将于补缴后的一周内开放！`,
        });
        return;
      }
      window.open(item);
    ...
    getDetail() {
      ...
      getDetail(data).then((res) => {
        if (res.code === 0) {
          ...
          // 待授权
          if (
            res.data.nodeCode !== this.nodeEnum.WAIT_UPLOAD &&
            res.data.nodeCode !== this.nodeEnum.WAIT_REVIEW &&
            [16, 31, 3101, 3102, 3103, 3104, 3105, 3106, 3107, 3108].includes(this.orderServices.productFunctionCode)
          ) {
          ...
          if (this.status == this.nodeEnum.REGISTER_COMPLETE || this.status == this.nodeEnum.REGISTERING) {
            ...
            (this.status == this.nodeEnum.AUTHORIZE_WAIT_CONFIRM && this.orderServices.progressBar == 2)
          ) {
            this.pageType = 3;
          }
          ...
          this.getRegisterHideInfo(res.data.orderServicesVO.serviceNo);
        }
      });
    },
    /** 初始化字段模板 */
    ...
    getRegisterHideInfo(serviceNo) {
      getHideInfo({ serviceNo }).then((res) => {
        if (res.code === 0) {
          if (res.data && res.data.payment == '否') {
            this.hideInfo = true;
          }
        }
      });
    ...
    formSave(materialData, next) {
      ...
      if ([31, 3101, 3102, 3103, 3104, 3105, 3106, 3107, 3108].includes(Number(this.orderServices.productFunctionCode)))
        requestFunc = EPRSave;
      requestFunc({
        serviceId: this.serviceId,
    ...
    formsubmit(materialData) {
      let requestFunc = authAgentCommit;
      if ([31, 3101, 3102, 3103, 3104, 3105, 3106, 3107, 3108].includes(Number(this.orderServices.productFunctionCode)))
        requestFunc = EPRCommit;
      requestFunc({
        serviceId: this.serviceId,
        materialData: materialData,
      })
    ...
    if (this.$store.state.app.stepIndex == 2) {
      this.$Modal.confirm({
        ...
        onOk: () => {
          ...
          let requestFunc = authAgentSave;
          if (
            [31, 3101, 3102, 3103, 3104, 3105, 3106, 3107, 3108].includes(
              Number(this.orderServices.productFunctionCode)
            )
          )
            requestFunc = EPRSave;
          requestFunc({
