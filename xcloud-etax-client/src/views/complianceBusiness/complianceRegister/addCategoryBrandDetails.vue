<template>
  <div>
    <div class="head">
      <div>
        <div class="fill-information-title">
          <img
            @click="$router.push('/complianceBusiness/addCategoryBrandList')"
            src="@/assets/images/common/dynamicForm/icon_fanhui2x.png"
          /><span class="left">加类加品牌详情</span>
          <span class="right">（ {{ serviceName }} ）</span>
          <Button @click="seeHistoryBuyInfo">查看历史购买产品信息</Button>
        </div>
        ...
        <div class="tabTitle">
          <span
            ...
            v-if="status == nodeEnum.REGISTER_COMPLETE"
            @click="pageType = 1"
          >
            申请信息
          </span>
    ...
    <Modal v-model="historyBuyInfoDialog" width="860" footer-hide title="查看历史购买产品信息">
      <div class="tipsModal-title" v-if="orderServices.productFunctionCode == 106">WEEE产品信息</div>
      <Table
        border
        :columns="countryCode === 'DE' ? columnsHisotryBuyDE  : columnsHisotryBuy"
        :data="historyBuyInfoList"
        v-if="orderServices.productFunctionCode == 106"
      >
      </Table>

      <Table
        border
        :columns="columnsBatHisotryBuy"
        :data="historyBuyInfoList"
        v-if="orderServices.productFunctionCode == 107"
      >
        <template slot-scope="{ row }" slot="unitWeight">
          {{ row.unitWeight }} {{ row.unitWeight == '1KG以上' ? '/' : '' }} {{ row.batteryWeight }}
        </template>
      </Table>
      
      <div style="height:20px;"></div>
      <!-- <BatteryProductDtos :value="historyBuyInfoList" v-if="type == 3" :disabled="true" /> -->
    </Modal>
  </div>
</template>

<script>
import { getDetail, getNodeField, queryHistoryList, queryBatHistoryList, getEprHistoryInfo } from '@/api/taxRegister.js';
...
import { EPRTaxServicesNodeList } from './addDateList';
import isServiceCancel from '@/views/home/mixin/isServiceCancel';
import {columnsHisotryBuy,columnsHisotryBuyDE} from './eprDeclareTableConfig';
// import BatteryProductDtos from '@/views/complianceBusiness/components/batteryProductDtos';
...
export default {
      ...
      this.moduleList.forEach((pro) => {
        pro.columns.forEach((item) => {
          ...
          if (
            ...
            ['weeeInfos', 'batteryInfos'].includes(item.keyName)
          ) {
            disabled = false;
          } else {
            disabled = true;
  ...
  methods: {
    ...
    seeHistoryBuyInfo() {
      let fnName = '';
      console.log(this.orderServices.serviceName);
      // 德国weee
      if(this.orderServices.productFunctionCode == 106 && this.orderServices.countryCode === 'DE' && this.info.orderServicesVO.parentProductFunctionCode === 3108 ){
        fnName = getEprHistoryInfo
      }else      if (this.orderServices.productFunctionCode == 106) {
        fnName = queryHistoryList;
      } else if (this.orderServices.productFunctionCode == 107) {
        fnName = queryBatHistoryList;
        console.log('?333');
      }

      fnName({
        serviceId: this.eprWorkOrderInfo.serviceId,
        year: moment(this.eprWorkOrderInfo.eprEffectiveDate || this.info.orderServicesVO.createTime).format('YYYY'),
        serviceNo: this.eprWorkOrderInfo.parentServiceNo,
        checkPassTime: this.eprWorkOrderInfo.checkPassTime || moment().format('YYYY-MM-DD HH:mm:ss'),
      }).then((res) => {
        if (res.code === 0) {
    ...
    getDetail() {
      ...
      getDetail(data).then((res) => {
        if (res.code === 0) {
          ...
          this.countryCode = res.data.orderServices.countryCode
          this.info.countryCode = res.data.orderServices.countryCode
      ...
      EPRCommit({
        ...
        .then((res) => {
          if (res.code == 0) {
            ...
            localStorage.backPath = '/complianceBusiness/addCategoryBrandList';
