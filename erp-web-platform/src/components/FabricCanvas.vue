<template>
  <div class="water-mark-wrap">
    <div class="operation-left-wrap">
      ...
      <div class="layer-common-operation" v-if="operationType">
        <div class="sub-operation">
          ...
          <a-button small @click="handleChange('flipX')" :disabled="isLock">
            {{ $t('other.horizontal') }}
          </a-button>
          <a-button small @click="handleChange('flipY')" :disabled="isLock">
            {{ $t('other.vertical') }}
          </a-button>
        </div>
        <div class="sub-operation">
          <span class="operation-title"> {{ $t('other.layer') }}： </span>
          <a-button @click="changeZindexHandle('top')" :disabled="isLock">{{
            $t('other.frontLayer')
          }}</a-button>
          <a-button @click="changeZindexHandle('back')" :disabled="isLock">{{
            $t('other.backLayer')
          }}</a-button>
          <a-button
            small
            ...
            :disabled="isLock"
            >{{ $t('other.upLayer') }}</a-button
          >
          <a-button
            small
            @click="changeZindexHandle('down')"
            :disabled="isLock"
            >{{ $t('other.downLayer') }}</a-button
          >
        </div>
        <div class="sub-operation">
          <span class="operation-title"> {{ $t('other.action') }}： </span>

          <a-button small @click="handleLockLayer()">{{
            isLock ? $t('other.unLock') : $t('other.lock')
          }}</a-button>
          <a-button small @click="handleCopyLayout()" :disabled="isLock">
            {{ $t('common.copy') }}
          </a-button>
          <a-button small @click="handleRemoveLayout()" :disabled="isLock">
            {{ $t('common.delete') }}
          </a-button>
        </div>
      </div>
    ...
    <div class="water-mark-example">
      <div
        style="background:url('https://sw-erp.oss-accelerate.aliyuncs.com/2021/04/21/1619005868633528667.png')"
      >
...
export default {
  ...
  watch: {
    ...
    activeObjectData: {
      handler() {
        this.isLock = this.canvas ? this.canvas.getActiveObject().isLock : false
        // this.handleLockLayer()
      },
      immediate: true
    }
  },
  ...
  methods: {
    render() {
      ...
      this.canvas.on({
        ...
        'mouse:down': this.updateControls
      })
    },
      ...
      this.$nextTick(() => {
        switch (key) {
          case 'charSpacing': //字符间距
            active.set(
              'charSpacing',
              this.activeObjectData['charSpacing'] * 1e3
            )
            break
          case 'scale': //缩放
            active
            ...
            break
          case 'flipX': //水平翻转
            active.set('flipX', !this.activeObjectData[key])
            this.activeObjectData.flipX = !this.activeObjectData.flipX
            break
          case 'flipY': //垂直翻转
            active.set('flipY', !this.activeObjectData[key])
            this.activeObjectData.flipY = !this.activeObjectData.flipY
            break
          default:
            active.set(key, this.activeObjectData[key])
            break
        }
    ...
    handleLockLayer() {
      ...
      if (active) {
        ...
        active.set('lockMovementX', this.isLock)
        active.set('lockMovementY', this.isLock)
        active.set('lockScalingX', this.isLock)
        active.set('lockScalingY', this.isLock)
        active.set('isLock', this.isLock)
        active.set('hasControls', !this.isLock)
        active.set('hasBorders', this.isLock)
        // active.set('evented', this.isLock)
        // active.set('selectable', this.isLock)
    ...
    changeZindexHandle(type) {
      let level = this.canvas.getActiveObject() // 获取当前被选中的元素
      switch (type) {
        case 'up':
          this.canvas.bringForward(level) // 上移
          break
        case 'down':
          this.canvas.sendBackwards(level) // 下移
          break
        case 'top':
          this.canvas.bringToFront(level)
          break
        case 'back':
          this.canvas.sendToBack(level)
          break
        default:
          break
      }
