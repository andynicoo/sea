    ...
    <div class="me-card">
      <a-space>
        <a-button
          class="me-handle-btn"
          type="primary"
          @click="openPage('/discount/edit-set-meal')"
          >新建活动</a-button
        >
      </a-space>
      <DiscountTable
...
export default {
  ...
  mixins: [dataBrandMxins, tableMixins],
  computed: {
    ...mapState({
  ...
  methods: {
    ...
    getTableData() {
      ...
      return Discount.getSetmealList({
        ...searchParams,
        size,
        current,
    ...
    async handleDispatchAction() {
      ...
      let data = await Discount.syncSetmealList(params).catch(() => {
    ...
    deleteDiscount({ id }) {
      this.$confirm({
        ...
        content: '确定删除吗?',
        onOk: async () => {
          await Discount.deleteSetmealDetail({ id })
    ...
    endDiscount({ id }) {
      this.$confirm({
        ...
        content: '确定终止吗?',
        onOk: async () => {
          await Discount.endSetmealDetail({ id })
          this.$message.success(this.$t('common.handlerSuccess'))
          this.reLoadData()
        }
      })
    ...
    editDiscount({ id, status }) {
      this.openPage(
        `/discount/edit-set-meal?edit=edit&id=${id}&status=${status}`
      )
    },
    viewDetail({ id, status }) {
      this.openPage(
        `/discount/edit-set-meal?edit=view&id=${id}&status=${status}`
      )
    }
  }
}
