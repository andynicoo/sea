    ...
    <div class="me-card">
      <a-space>
        <a-button
          class="me-handle-btn"
          type="primary"
          @click="openPage('/discount/edit-coupon')"
          >创建优惠券</a-button
        >
      </a-space>
      <DiscountTable
...
export default {
  ...
  methods: {
    ...
    getTableData() {
      ...
      return Discount.getVoucherList({
        ...searchParams,
        size,
        current,
    ...
    async handleDispatchAction() {
      ...
      let data = await Discount.syncVoucherList(params).catch(() => {
    ...
    deleteDiscount({ id }) {
      this.$confirm({
        ...
        content: '确定删除吗?',
        onOk: async () => {
          await Discount.deleteCouponDetail({ id })
    ...
    endDiscount({ id }) {
      this.$confirm({
        ...
        content: '确定终止吗?',
        onOk: async () => {
          await Discount.endCouponDetail({ id })
          this.$message.success(this.$t('common.handlerSuccess'))
          this.reLoadData()
        }
      })
    ...
    editDiscount({ id, status }) {
      this.openPage(`/discount/edit-coupon?edit=edit&id=${id}&status=${status}`)
    },
    viewDetail({ id, status }) {
      this.openPage(`/discount/edit-coupon?edit=view&id=${id}&status=${status}`)
    }
  }
}
</script>
