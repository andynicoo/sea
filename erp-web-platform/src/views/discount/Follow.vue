    ...
    <div class="me-card">
      <a-space>
        <a-button
          class="me-handle-btn"
          type="primary"
          @click="openPage('/discount/edit-fllow')"
          >创建优惠券</a-button
        >
      </a-space>
      <DiscountTable
...
// import AsyncGoods from '../goods/components/AsyncGoods'
...
export default {
  ...
  methods: {
    ...
    getTableData() {
      ...
      return Discount.getFlollowList({
        ...searchParams,
        size,
        current,
    ...
    async handleDispatchAction() {
      ...
      let data = await Discount.syncFollowList(params).catch(() => {
    ...
    deleteDiscount({ id }) {
      this.$confirm({
        ...
        content: '确定删除吗?',
        onOk: async () => {
          await Discount.deleteFollowDetail({ id })
    ...
    endDiscount({ id }) {
      this.$confirm({
        ...
        content: '确定终止吗?',
        onOk: async () => {
          await Discount.endFollowDetail({ id })
          this.$message.success(this.$t('common.handlerSuccess'))
          this.reLoadData()
        }
      })
    },
    editDiscount({ id, campaignStatus }) {
      this.openPage(
        `/discount/edit-fllow?edit=edit&id=${id}&status=${campaignStatus}`
      )
    },
    viewDetail({ id, campaignStatus }) {
      this.openPage(
        `/discount/edit-fllow?edit=view&id=${id}&status=${campaignStatus}`
      )
    }
  }
}
