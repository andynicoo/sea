...
const LIMIT_LINE = process.argv[2] || 1000
...
getFiles.getTxtList('./src/').forEach(function(item) {
  //在这里输入你要统计的文件夹名（记得在后面加斜线）
  ...
  len += conArr.length
  if (fileRowNumber > LIMIT_LINE) {
    console.log(filePath) //打印行数大于LIMIT_LINE的文件名
    bigFileCount++
  }
  fileCount++
})
...
console.log(`超过${LIMIT_LINE}行代码的文件:`, bigFileCount)
