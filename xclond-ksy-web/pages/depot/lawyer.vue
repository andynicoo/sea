  ...
  <div class="lawyer">
    ...
    <div class="content">
      ...
      <div class="nav">
        ...
        <div v-for="(item, index) in rankList" :key="index" :class="{ item: true }">
      ...
      <div class="section layerbox">
        <div class="item" v-for="(item, index) in rankList" :key="index">
          ...
          <a-table
            :columns="columns"
            :data-source="item.attorneyInfo.slice(0, 10)"
            :pagination="false"
            :rowKey="
              (record, index) => {
                return index;
          ...
          <nuxt-link class="more-btn" :to="'/depot/lawyerDetails?year=' + item.years">
            <div>展开更多</div>
            <span></span>
          </nuxt-link>
        </div>
...
export default {
  ...
  methods: {
    ...
    changeYear(year) {
      this.$router.push({
        path: "/depot/lawyerDetails?year=" + year,
      });
    },
    enterDetails(val) {
      this.$router.push({
