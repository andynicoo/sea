  ...
  <div>
    ...
    <div class="center ksy-w1320">
        <SearchBoxNew
          ...
          placeholder="请输入您要查询的申请人名称"
          ref="searchbox"
          page="brandPersonnel"
          :exampleList="defaultOwner"
          :isFuzzyMatch="true"
      ...
      <div class="agency-static-wrap">
        ...
        <div class="agency-static-main flex flex-b">
          ...
          <div style="width: 383px">
            <StaticColumn :data="columnData.slice(0, 6)"></StaticColumn>
          </div>
          <div style="width: 215px">
            <StaticPie :data="pieData.slice(0, 4)" left="50%"></StaticPie>
          </div>
        </div>
      </div>
      <filter-height
...
export default {
  ...
  data() {
    this.getFuzzyMatchList = debounce(this.getFuzzyMatchList, 500);
    return {
      loading: null,
      form: {
        countryList: [],
  ...
  computed: {
    ...mapState("trademark", ["defaultOwner"]),
  },
  methods: {
    timeChange(val) {
    ...
    // 统计数据
    async getAnalysis() {
      const res = await this.$axiosApi(trademark.getAnalysis.url, trademark.getAnalysis.methods, {
        country: "US", // 默认美国
        applyDateStr: this.rangeDate.join(","),
        ownerNameList: [this.form.cruTrade],
        aggregationSize: 6,
      });
      if (res.code === 0) {
        this.lineData = res.data.trademarkAttorneyTrendDTOList;
        this.columnData = res.data.attorneyList;
        this.pieData = res.data.categoryList;
    ...
    updateData() {
      this.$nextTick(async () => {
        ...
        await this.checkTrademarksName("ownerName");
    ...
    async getFuzzyMatchList(val) {
      ...
      const { data } = await this.$axiosApi(trademark.getOwnerList.url, trademark.getOwnerList.methods, {
        keyWord: val,
        pageIndex: 1,
        pageSize: 20,
      });
