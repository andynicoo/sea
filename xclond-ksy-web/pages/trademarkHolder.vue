      ...
      <div class="box-head">
        ...
        <div class="box-head-extend">
          <ul class="flex flex-c-c">
            <li
              class="flex flex-c-c"
              v-for="(item, index) in ['申请商标量统计', '申请商标状态分布', '地区分布', '代理律师TOP6']"
              :key="index"
            >
              {{ item }}
            </li>
        ...
        <search-page @search="search" placeholder="请输入您要查询的申请人名称">
          <template #tips
...
export default {
    ...
    try {
      let [{ data: bigBanner }, { data: navList }, { data: footerList }, { data: webInfo }] = await Promise.all([
        context.$axiosApi(common.getAdByAdPositionKey.url, common.getAdByAdPositionKey.methods, {
          adKey: "The_applicant_holds_a_trademark_top_banner",
        }),
  ...
  data() {
    return {
      ...
      titleImg: require("@/assets/images/trademark/v230401/title/owner.png"),
      quickStr: ["Apple Inc.", "ALFREDO ZABALA"],
    };
  },
  methods: {
    async search(val) {
